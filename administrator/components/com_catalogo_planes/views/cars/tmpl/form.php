...
$deleteDeliveryCityImg = '<img border="0" class="deletedeliverycity" alt="' . JText::_('CP.DELETE') . '" src="' . COM_CATALOGO_PLANES_IMAGESDIR . '/cancel.png" />';
...
<script type="text/javascript">
	...
	jQuery.noConflict()(document).ready(function ($) {
        ...
        if ($data->product_id < 1) {
        	echo '$(\'#details_disclaimer\').val("' . JText::_('CP.CAR_DISCLAIMER_DEFAULT_VALUE') . '");';
        }
        ?>
        ...
        var elementsOrder = ["product_name", "detailscountry_id", "detailsregion_id", "detailscity_id", 
                             "detailszone_id", "details_publish_up", "details_publish_down", 
                             "details_featured", "details_product_desc", "details_product_code", 
                             "details_disclaimer", "details_access", 
                             "detailscategory_id", "detailssupplier_id", "details_published1", 
                             "details_published0", "details_product_url", "selecttaxes", "selectdeliverycities"];
        ...
        $("#supplement_product_types,#supplement_tourism_types").change(function() {
            ...
            generateSelectByAjax(url, 'supplement_list', 'supplement_id', 'supplement_name', '0', emptySelectOption, assignedSupplementList, "supplement_search_message", "<?php echo JText::_('CP.CAR_SUPPLEMENT_SEARCH_NO_RESULTS_MESSAGE', true); ?>");
        });
        ...
        $("#selectdeliverycities").autocomplete({
            source: function(request, response) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.length > 0) {
	                        // Agregar SÓLO si la ciudad no se haya relacionado al producto.
	                        arr = $.grep(data, function(item, index) {
	                            return ($('div.citiescontent input:hidden[value="' + item.city_id + '"]').length < 1);
	                        });
	                        if (arr.length > 0) {
            ...
            select: function(event, ui) {
                ...
                if (ui.item.value != 0) {
	                $('div.citiescontent table').append('<tr><td><input type="hidden" name="deliverycities[]" id="deliverycities[]" value="' + ui.item.value + '" /><span>' + ui.item.label + '</span></td><td align="right"><?php echo $deleteDeliveryCityImg; ?></td></tr>');

	                // Hacer que el panel de ciudades se pueda extender o encoger a medida que se agregar/eliminan
	                $('#delivery-cities-page ~ div.jpane-slider').css('height', 'auto');

	                // Limpiar el campo de búsqueda
	                $("#selectdeliverycities").val('');
                }
            },
        ...
        $("body").delegate(".deletedeliverycity", "click", function() {
            var row = $(this).closest("tr");
...
Joomla.submitbutton = function(pressbutton) {
        ...
        if (jQuery.trim(form.product_name.value) == "") {
            ...
            alert("<?php echo JText::_('CP.CAR_ERROR_EMPTY_CARCATEGORY', true); ?>");
        ...
        <tr>
            <td valign="top">
                <table border="0">
                    ...
                    <tr>
                        ...
                        <?php else: ?>
                            <input type="text" name="detailsregion_id" id="detailsregion_id" value="<?php echo $data->region_id; ?>" />
                        <?php endif; ?>
                        </td>
                        <td align="right" class="key2">
                            <label for="city">
            ...
            <td valign="top">
               ...
               foreach($this->form->getFieldset() as $field){
                ...
                // Se inicia el panel de ciudades de entrega
                $selectedCities = '';
                $content .= $pane->startPanel(JText::_('CP.CAR_DELIVERY_CITY_NAME_LABEL'), "delivery-cities-page");
                // Generar listado de ciudades ya asignadas
				foreach ($data->deliverycities as $key => $row) {
                    // El nombrel hotel debe contener asterisco si está inactivo
                    $row->city_name .= ($row->published != 1)? '*&nbsp;': '&nbsp;';
                    $selectedCities .= '<tr><td><input type="hidden" name="deliverycities[]" id="deliverycities[]" value="' . $row->city_id . '" /><span>' . $row->city_name . '</span></td><td align="right">' . $deleteDeliveryCityImg . '</td></tr>';
                }
                $content .= '<div id="system-message"><div class="info"><ul><li>' . JText::_('CP.CAR_DELIVERY_CITY_DESCRIPTION') . '</li></ul></div>';
                $content .= '<div class="citiescontent"><table border="0" cellspacing="5"><tr><td colspan="2">';
                $content .= '<input class="text_area" type="text" name="selectdeliverycities" id="selectdeliverycities" size="40" />';
                $content .= '</td></tr>' . $selectedCities . '</table>' . $pane->endPanel();
...
?>
        <fieldset>
            <legend><?php echo JText::_('CP.CAR_SUPPLEMENT_SEARCH_LABEL'); ?></legend>
...
?>
    ...
    <input type="hidden" name="rateparam1[]" value="1" />
    <input type="hidden" name="rateparam2[]" value="1" />
    <input type="hidden" name="rateparam2[]" value="2" />
    <input type="hidden" name="delMedia" value="" />
