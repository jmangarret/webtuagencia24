...
* @subpackage		catalogo_planes
*/
class TableCarCategory extends BasicTable {
	...
	function __construct(& $db) {
		parent::__construct('#__cp_prm_cars_category', 'category_id', $db);
	}
	...
	function check() {
		...
		if (!$this->category_name) {
			$this->setError(JText::_('CP.CARCATEGORY_ERROR_EMPTY_NAME'));
			return false;
		}
		...
		if ($id) {
			$this->setError(JText::_('CP.CARCATEGORY_ERROR_DUPLICATE_NAME'));
			return false;
		}
	...
	function delete($oid = null) {
        ...
        if ($id) {
        	...
        	$query = 'SELECT product_id FROM #__cp_cars_info WHERE category_id = ' . $id . ' LIMIT 1';
        	$this->_db->setQuery($query);
        	$obj = $this->_db->loadObject();
        	if ($obj) {
        		$this->setError(JText::sprintf('CP.CARCATEGORY_ERROR_DELETE_EXISTS_RELATED_PRODUCTS', $id));
        		return false;
        	}
		}
