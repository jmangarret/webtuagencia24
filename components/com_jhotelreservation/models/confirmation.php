...
class JHotelReservationModelConfirmation extends JModelLegacy
{
	...
	function saveConfirmation($reservationDetails){
		try{
			...
			/*---------------------------------------------------------------------------------*/
			/*MICOD
			En éste bloque es donde se almacena en la base de datos las habitaciones reservadas.
			Fué modificada para que pueda contar las edades en la variable "$edades_habitacion"
			y lo enviara por parámetro a la función*/
			$edades_habitacion = $reservationDetails->reservationData->userData->child_ages; //Donde reasigno las edades de los nios por habitación
			foreach($reservationDetails->rooms as $key => $room){ //Modificado: He agregado el "$key" para posicionar las habitaciones
				$confirmationRoomId = $this->storeConfirmationRooms($reservaitonId, $room, $edades_habitacion[$key]); //Se le anexa el nuevo pase por parámetro
	...
	function storeConfirmation($reservationDetails){
		...
		/*----------------------------------------------------------------*/
		//MICOD
		//Aquí asigno las edades de los niños a la base de datos, solo tomando un $obj-> a que apunte a un campo de la base de datos
		//luego le asigno el valor que deseo guardar.
		$obj->child_age					= 	$reservationDetails->reservationData->userData->child_age;
		/*----------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------*/
	/*MICOD
	Ésta función fué modificada con un nuevo valor por parámetro que representa las edades
	de los niños por habitación*/
	function storeConfirmationRooms($confirmationId, $room, $edades_habitacion){ //Nuevo parámetro para las edades
		...
		$obj->child_ages        =   $edades_habitacion; //Pasamor el valor al objeto para guardarlo en la base de datos
		//dmp($obj);
		//exit;
		if (!$rowTable->bind($obj)){
			throw( new Exception($this->_db->getErrorMsg()) );
