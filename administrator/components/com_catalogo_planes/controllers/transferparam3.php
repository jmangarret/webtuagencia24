...
/**
 * transferparam3 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTransferParam3 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('transferparam3');
		$view = & $this->getView('transferparam3', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param3_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=transferparam3', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=transferparam3';
		if ($this->task == 'apply') {
			$param3_id = JRequest::getInt('param3_id');
			if ($param3_id < 1) {
				$data =& $model->getRow();
				$param3_id = $data->param3_id;
			}
			$link .= '&task=edit&cid[]=' . $param3_id;
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=transferparam3', $msg, $type);
	}
