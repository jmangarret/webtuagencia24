 ...
 *
 * @version $Id: city.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelCity extends CatalogoPlanesModelBasic {
	...
	function __construct() {
		...
		$this->_filter_prefix = $option . '_city';
    }
	...
	protected function _buildQuery($use_filter = true, $conditions = array()) {
		...
		$orderby	= ($use_filter && (count($conditions) < 1))? $this->_buildQueryOrderBy(): ' ORDER BY a.city_name ASC, b.country_name ASC, c.region_name ASC';

		$query = ' SELECT a.*, b.country_name, c.region_name FROM ' . $this->getTable()->getTableName();
		$query .= ' a JOIN #__cp_prm_country b ON a.country_id = b.country_id ';
		$query .= ' LEFT JOIN #__cp_prm_region c ON a.region_id = c.region_id ' . $where . $orderby;
	...
	protected function _buildQueryOrderBy() {
		...
		switch ($filter_order) {
			case 'country_name':
				$filter_order = "b.country_name $filter_order_Dir, c.region_name ASC, a.city_name ASC";
				break;
			case 'region_name':
				$filter_order = "c.region_name $filter_order_Dir, b.country_name ASC, a.city_name ASC";
				break;
			default:
				$filter_order = "a.$filter_order $filter_order_Dir, b.country_name ASC, c.region_name ASC";
				break;
		}
	...
	public function getOrderByField() {
		...
		if (!in_array($filter_order, array('city_id', 'city_name', 'city_code', 'ordering', 'country_name', 'region_name', 'published'))) {
	...
	protected function _buildQueryWhere($conditions = array()) {
		...
		if (count($conditions) > 0) {
			...
			$region_id = $mainframe->getUserStateFromRequest($this->_filter_prefix . 'region_id', 'filter_region_id', '', 'int');
			...
			if ($search) {
				$where[] = ' LOWER(a.`city_name`) LIKE ' . $this->_db->Quote('%' . $this->_db->getEscaped($search, true).'%', false);
			}
			if ($country_id) {
