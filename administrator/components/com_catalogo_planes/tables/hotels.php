...
* @subpackage		catalogo_planes
*/
class TableHotels extends BasicTable {
	...
	function check() {
		...
		if ($id) {
			$this->setError(JText::_('CP.HOTEL_ERROR_DUPLICATE_NAME'));
			return false;
		}
		...
		$id = $this->_db->loadResult();
		if ($id) {
			$this->setError(JText::_('CP.HOTEL_ERROR_DUPLICATE_CODE'));
			return false;
		}
    ...
    function duplicate($cid) {
        if ($cid > 0) {
            ...
            $query = 'SELECT CLONEHOTEL(' . $cid . ', \'' . JText::_('CP.PRODUCT_DUPLICATE_NAME_PREFIX') . '\', ' . $user->get('id') . ', \''.date("mdHis").'\')';
            $this->_db->setQuery($query);
