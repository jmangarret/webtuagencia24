 ...
 *
 * @version $Id: cardeliverycity.php 2012-09-11 8:29:50 svn $
...
/**
 * cardeliverycity Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerCarDeliveryCity extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('cardeliverycity');
		$view = & $this->getView('cardeliverycity', 'html');
	...
	function edit() {
		...
		if ($cid[0] && empty($data->city_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=cardeliverycity', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=cardeliverycity';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=cardeliverycity', $msg, $type);
	}
	...
	/**
	 * Obtiene listado de ciudades por nombre
	 */
	function getRawListByName() {
		...
		// Recupera los valores de los filtros
		$city_name = JRequest::getVar('city_name', '');

		// Recupera la información
		$data = $model->getListByName($city_name);
