	...
	function _getToggle($column,$table = ''){
		...
		if($column == 'published' AND !in_array($table,array('plugins','list'))){
			if(version_compare(JVERSION,'1.6.0','<')){
				$params->pictures = array(0=>'images/publish_r.png',1=>'images/publish_g.png',2=>NEWSLETTER_IMAGES.'schedule.png');
			}else{
	...
	function toggleText($action= '',$value='',$table='',$text=''){
		...
		if(!$jsincluded){
			...
			$js = "function joomToggleText(id,newvalue,table){
				...
				try{
					new Ajax('index.php?option=com_jhotelreservation&tmpl=component&controller=newslettertoggle&task='+id+'&value='+newvalue+'&table='+table,{ method: 'get', update: jQuery(id), onComplete: function() {	window.document.getElementById(id).className = 'loading'; }}).request();
				}catch(err){
					new Request({url:'index.php?option=com_jhotelreservation&tmpl=component&controller=newslettertoggle&task='+id+'&value='+newvalue+'&table='+table,method: 'get', onComplete: function(response) { jQuery(id).innerHTML = response; window.document.getElementById(id).className = 'loading'; }}).send();
				}
			}";
		...
		if($params->mode == 'pictures'){
			...
			if(!$classincluded){
				...
				$js = "function joomToggleClass(id,newvalue,table,extra){
					var mydiv=jQuery(id); mydiv.innerHTML = ''; mydiv.className = 'onload';
					try{
						new Ajax('index.php?option=com_jhotelreservation&tmpl=component&controller=newslettertoggle&task='+id+'&value='+newvalue+'&table='+table+'&extra[color]='+extra,{ method: 'get', update: jQuery(id), onComplete: function() {	window.document.getElementById(id).className = 'loading'; }}).request();
					}catch(err){
						new Request({url:'index.php?option=com_jhotelreservation&tmpl=component&controller=newslettertoggle&task='+id+'&value='+newvalue+'&table='+table+'&extra[color]='+extra,method: 'get', onComplete: function(response) { jQuery(id).innerHTML = response; window.document.getElementById(id).className = 'loading'; }}).send();
					}
					}";
				$doc =JFactory::getDocument();
				$doc->addScriptDeclaration( $js );
			...
			if(!empty($extra['tooltip'])) $return .= newsletter_tooltip($extra['tooltip'], @$extra['tooltiptitle'],'','&nbsp;&nbsp;&nbsp;&nbsp;');
			$return .= '</div></a>';
			return $return;
		}
	}
	...
	function delete($lineId,$elementids,$table,$confirm = false,$text = ''){
		...
		if(!$deleteJS){
			...
			$js = "function joomDelete(lineid,elementids,table,reqconfirm){
				if(reqconfirm){
					if(!confirm('".JText::_('HRN_VALIDDELETEITEMS',true)."')) return false;
				}
				try{
					new Ajax('index.php?option=com_jhotelreservation&tmpl=component&controller=".$this->ctrl.$this->extra."&task=delete&value='+elementids+'&table='+table, { method: 'get', onComplete: function() {window.document.getElementById(lineid).style.display = 'none';}}).request();
				}catch(err){
					new Request({url:'index.php?option=com_jhotelreservation&tmpl=component&controller=".$this->ctrl.$this->extra."&task=delete&value='+elementids+'&table='+table,method: 'get', onComplete: function() { window.document.getElementById(lineid).style.display = 'none'; }}).send();
				}
			}";
			$doc =JFactory::getDocument();
			$doc->addScriptDeclaration( $js );
		}
		if(empty($text)) $text = '<img src="'.NEWSLETTER_IMAGES.'delete.png"/>';
