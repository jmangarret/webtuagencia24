...
class HotelService{
	...
	/*MICOD
	He agregado un nuevo parámetro a la siguiente función para obtener el número de habitaciones que se genera desde el archivo "ReservationService.php"
	en la función "getSelectedRooms" de la línea 304
	También se anexó el parámetro "$reservationData" para los datos de las edades de los niños y la configuración por edad de cada hotel*/
	public static function getHotelRooms($hotelId, $startDate, $endDate, $roomIds=array(), $adults=2, $children=0, $discountCode = null, $checkAvailability = true,$confirmationId=null, $num_habit=0, $reservationData){
		...
		foreach($rooms as $room){
			...
			$totalPrice = 0;
			$currentDayNr =1; 
			$number_days = round((strtotime($endDate) - strtotime($startDate) ) / ( 60 * 60 * 24));
			...
			for( $d = strtotime($startDate);$d < strtotime($endDate); ){
				...
				if($room->price_type==1){
					...
					/*---------------------------------------------------------------------------------*/
					/*MICOD
					Aquí estoy calculando el FEE de los precios por adulto, útil para calcular
					tarifas de los niños según su edad*/
					$dayPrice_sin_fee = $dayPrice;//éste valor se utiliza para calcular el precio en porsentaje del niño antes del fee
					$comision = $room->commission;
					$total_comision_day = $dayPrice * $comision / 100;		
							
					$dayPrice = $dayPrice + $total_comision_day;
					$dayPrice = round($dayPrice, 2);
					/*---------------------------------------------------------------------------------*/
					...
					/*------------------------------------------------------------------------------------------------------------------------------------------------------------*/
					/*MICOD
					En éste bloque adquiero los datos de configuración de los hoteles para las tarifas de los niños segun la edad
					para luego compararlo con los datos de los huespedes y dar un precio final.*/

					//Los hubicamos y lo asignamos a comodas variables...
					$niños_libre_tarifas = $reservationData->hotel->informations->niños_libre_tarifas;
					
					$niños_tarifa_ajustada = $reservationData->hotel->informations->niños_tarifa_ajustada;
					
					$edad_tarifa_adult = $reservationData->hotel->informations->edad_tarifa_adult;
				
					if ($reservationData->userData->child_age == NULL) {
						$child_ages = $reservationData->userData->child_ages;
						$edades = explode("|", $child_ages[$num_habit]);
					}else{
						$child_ages = $reservationData->userData->child_age;
						$edades = explode("|", $child_ages);
					}
					
					//Los convertimos en un Array para posicionar cada dato que se va a utilizar...
					$niños_libre_tarifas = explode("|", $niños_libre_tarifas);

					$niños_tarifa_ajustada = explode("|", $niños_tarifa_ajustada);

					$edad_tarifa_adult = explode("|", $edad_tarifa_adult);


					//Realizamos un conteo de los niños (siempre 6 según ésta variable) para los ciclos for...
					$cont = count($edades);

					/*MICOD
					Reingeniería total al código de calculo de cotizaciones de los niños según su edad*/
					for ($i=0; $i < $cont; $i++) {  //Ciclo for que intera cada niño y su edad
						$check_gratis = false;
						$check_ajustada = false;
						$check_adutlo = false;
						$check_disable = false;
						if ($niños_libre_tarifas[2] == 1) { //Si la configuración de niños gratis está activa entonces...
							if (($edades[$i] != "") && ($edades[$i] != "Edad")) { //Solo pasar los niños registrados en la habitación...
								if (($edades[$i] >= $niños_libre_tarifas[0]) && ($edades[$i] <= $niños_libre_tarifas[1])) { //Condición dentro del rango de edad predefinido...
									$check_gratis = true; //Se confirma niño gratis
								}
							}
						}
						if ($niños_tarifa_ajustada[4] == 1) { //Si la configuración de niños con tarifa ajustada está activa entonces...
							if (($edades[$i] != "") && ($edades[$i] != "Edad")) {//Solo pasar los niños registrados en la habitación...
								if (($edades[$i] >= $niños_tarifa_ajustada[0]) && ($edades[$i] <= $niños_tarifa_ajustada[1])) {//Condición dentro del rango de edad predefinido...
									$check_ajustada = true; //Se confirma niño ajustado
								}
							}
						}
						if ($edad_tarifa_adult[1] == 1) { //Si la configuración de niños con precio de adultos está activa entonces...
							if (($edades[$i] != "") && ($edades[$i] != "Edad")) {//Solo pasar los niños registrados en la habitación...
								if ($edades[$i] >= $edad_tarifa_adult[0]) {//Condición dentro del rango de edad predefinido...
									$check_adutlo = true; //Se confirma niño adulto
								}
							}
						}
						...
						if (($check_gratis == false) && ($check_ajustada == false) && ($check_adutlo == false)) { //Si el niño no entró en ninguna condición entonces...
							if (($edades[$i] != "") && ($edades[$i] != "Edad")) { //Solo pasar los niños registrados en la habitación...
								$total_comision_child = $childPrice * $comision / 100;
								$childPrice = $childPrice + $total_comision_child;
								$childPrice = round($childPrice, 2); 
								$datos_niños_totales[$i] = $childPrice; //Se calcula el precio estandar el niño según la tarifa del hotel...
							}
						}elseif($check_gratis == true){ //Calculo de tarifa de niños gratis...
							$datos_niños_totales[$i] = 0; //Precio del niño siempre 0...
						}elseif ($check_ajustada == true) { //Calculo de tarifas de niños ajustados...
							if ($niños_tarifa_ajustada[3] == 1){//Si el dato en porsentaje está activo entonces...
								$datos_niños_totales[$i] = $dayPrice_sin_fee * $niños_tarifa_ajustada[2] / 100; //Obtenemos el precio del niño según el porcentaje del preciod el adulto
							}elseif($niños_tarifa_ajustada[3] == ""){//Si el dato en porcentaje está inactivo...
								$datos_niños_totales[$i] = $niños_tarifa_ajustada[2];//Obtenemos el precio previamente insertado en la configuración de cada hotel...
							}	
						}elseif ($edad_tarifa_adult[1] == 1) { //Calculo de tarifas niños adultos...
							$datos_niños_totales[$i] = $dayPrice;
						}
					}

					if (isset($datos_niños_totales)) { //Arreglo que carga con todas las tarifas de los niños para todos los casos
						foreach ($datos_niños_totales as $key => $value) {
							$total_niños += $value; //Se acumula en una sola variable
						}
					}
					$datos_niños_totales = 0; //IMPORTANTE: Limpiar arreglo después de usarlo para evitar anomalías en la tarifa final...
					$dayPrice = $dayPrice * $totalAdults + $total_niños; //Tarifa total de la habitación...
				...
				/*------------------------------------------------------------------------------*/
				//MICOD
				/*Ésta parte tiene MUCHO que ver con lo del Sumplemento individual
				*Si la habitación tiene una tarifa tipo "por habitación por noche" encontes
				*ésa cantidad se tomará como un descuento, en cuanto al tipo "por persona por noche"
				*ocurre el caso opuesto, mucho cuidado con los datos de las habitaciones en el administrator.
				*Ésto solo aplica cuando solo hay un huesped para cotizar
				*/
				...
				/*------------------------------------------------------------------------------*/
				//MICOD
				//Lugar donde tomo el precio de la habitación con tarifa tipo "Por Habitación por Noche"
				//aquí capturo ése dato y le calculo el FEE de la empresa.
				//Éste dato no va dirigido a la vista de las habitaciones, sino directamente a calculo final y a la cotización por email
				if($room->price_type==0){
					$comision = $room->commission;
					$total_comision = $dayPrice * $comision / 100;
					$dayPrice = $dayPrice + $total_comision;
				}
				/*------------------------------------------------------------------------------*/
			...
			$comision = $room->commission;
			$total_comision_per_room = $room->room_average_price * $comision;
			//$room->room_average_price = $room->room_average_price + $total_comision_per_room;
			//$room->room_average_price = round($room->room_average_price, 2);

			/*dmp($room->room_average_price);
			dmp($total_comision_per_room);
			dmp($comision);*/
			...
			//---------------------------------------------------------------------------------
			//micod
			//CÓDIGO MODIFICADO, AQUÍ OBLIGO A QUE SE MUESTRE EL PRECIO REAL DE LA HABITACIÓN, EL CUAL UTILIZA PARA COTIZAR

			//micod
			//Anexando el FEE en el precio POR PERSONA de las habitaciones, SOLO A LOS ADULTOS

			$comision = $room->commission;
			$precio = $room->$string_price;
			$total_comision = $precio * $comision / 100;
			$room->pers_total_price = $room->$string_price + $total_comision;
			$room->pers_total_price = round($room->pers_total_price, 2); //Redondear el precion decimal a solo 2 cifras
			$room->pers_total_price = number_format($room->pers_total_price, 2);

			//CÓDIGO ORIGINAL QUE MUESTRA UN PRECIO DIFERNTE EN LA PANTALLA DE LAS HABITACIONES DEL HOTEL
			//EL PROBLEMA ES QUE POR CADA NIÑO QUE EL CLIENTE ANEXA, EL PRECIO DEL ADULTO BAJA Y NO DEBERÍA PASAR.
			//$room->pers_total_price = JHotelUtil::fmt($totalPrice/($adults+$children),2); 

			/*$precio_total_niños = $childPrice*$children;
			$totalPrice = $totalPrice-$precio_total_niños;
			$var = $totalPrice/($adults);
			$var = number_format($var, 2);
			//$room->pers_total_price = JHotelUtil::fmt($totalPrice/($adults+$children),2); 
			$room->pers_total_price = $var;*/
			//dmp($room->$string_price);
			//Esto solo tiene efecto en la vista, donde se seleccionan las habitaciones, no influye en la cotización final
			//---------------------------------------------------------------------------------
						...
						/*	echo "<pre>";
							var_dump($precio);
							var_dump($comision);
							var_dump($total_comision);
							var_dump($room->pers_total_price);
							echo "</pre>";*/
			//dmp($room);
