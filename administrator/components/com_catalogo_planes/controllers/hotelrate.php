...
/**
 * hotelrate Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotelRate extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('hotelrate');
		$modelSeason =& $this->getModel('season');
		$view = & $this->getView('hotelrate', 'html');
		...
		$modelProduct =& $this->getModel('hotels');
	...
	function edit() {
		...
		if ($rate_id && !$data->season_id) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotelrate&product_id=' . $product_id, JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		...
		if ($rate_id > 0) {
			$seasons = $modelSeason->getSeasonsByProductType('hotels', $data->season_id, $product_id, $rate_id);
			$data->dates = $modelSeason->getSeasonDates($data->season_id);
		} else {
			$seasons = $modelSeason->getSeasonsByProductType('hotels', 0, $product_id);
			$data->dates = array();
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotelrate&product_id=' . $product_id;
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$product_id = JRequest::getInt('product_id');
		$this->setRedirect('index.php?option=' . $option . '&view=hotelrate&product_id=' . $product_id, $msg, $type);
	}
