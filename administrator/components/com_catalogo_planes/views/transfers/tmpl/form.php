...
<script type="text/javascript">
	...
	jQuery.noConflict()(document).ready(function ($) {
        ...
        var a = "<a>test</a>";
        $('#details_latitude').parent().append('&nbsp;<a href="javascript: void(0)" id="sendCoordinates"><?php echo JText::_("CP.PRODUCT_SEND_COORDINATES")?></a>');
        ...
        $("#sendCoordinates").click(function() {
            ...
            var regexLat = /\-?[0-9]+[\.]{0,1}[0-9]*/;
            var latitude = $('#details_latitude').val();
            var longitude = $('#details_longitude').val();
            if (latitude.length > 0 && (!regexLat.test(latitude) || latitude < -90 || latitude > 90)) {
                $('#details_latitude').focus();
                alert("<?php echo JText::_('CP.PRODUCT_ERROR_INVALID_LATITUDE', true); ?>");
                return;
            }
            if (longitude.length > 0 && (!regexLat.test(longitude) || longitude < -180 || longitude > 180)) {
                $('#details_longitude').focus();
                alert("<?php echo JText::_('CP.PRODUCT_ERROR_INVALID_LONGITUDE', true); ?>");
                return;
            }
            var latlng = new google.maps.LatLng(latitude, longitude);
...
Joomla.submitbutton = function(pressbutton) {
        ...
        if (jQuery.trim(form.product_name.value) == "") {
            ...
            // Validar las coordenadas ingresadas
            var regexLat = /\-?[0-9]+[\.]{0,1}[0-9]*/;
            var latitude = jQuery('#details_latitude').val();
            var longitude = jQuery('#details_longitude').val();
            if (latitude.length > 0 && (!regexLat.test(latitude) || latitude < -90 || latitude > 90)) {
                jQuery('#details_latitude').focus();
                alert("<?php echo JText::_('CP.PRODUCT_ERROR_INVALID_LATITUDE', true); ?>");
                return;
            }
            if (longitude.length > 0 && (!regexLat.test(longitude) || longitude < -180 || longitude > 180)) {
                jQuery('#details_longitude').focus();
                alert("<?php echo JText::_('CP.PRODUCT_ERROR_INVALID_LONGITUDE', true); ?>");
                return;
            }
