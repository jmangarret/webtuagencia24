...
* @subpackage		catalogo_planes
*/
class TablePlanRate extends BasicTable {
	...
	function __construct(& $db) {
        ...
        $this->relations['prices']['referencedtable'] = '#__cp_plans_rate_price';
        ...
        $this->relations['supplements']['referencedtable'] = '#__cp_plans_rate_supplement';
        $this->relations['supplements']['referencedfield'] = 'rate_id';
        $this->relations['supplements']['extrainfofields'] = array('amount');

        parent::__construct('#__cp_plans_rate', 'rate_id', $db);
	}
    ...
    public function load($id = null, $loadRelationships = true) {
        ...
        $query .= ' a JOIN #__cp_plans_info p ON a.product_id = p.product_id JOIN';
    ...
    function store($updateNulls = false, $updateRelationships = true) {
        ...
        if ($result && $this->product_id) {
	        ...
	        $calculatePrices = 0;
            $query = 'CALL GeneratePlansRateResumeById(' . $this->product_id . ', CURDATE(), ADDDATE(CURDATE(), 365), ' . $calculatePrices . ')';
            $this->_db->setQuery($query);
            if (!$this->_db->query()) {
            	$this->setError($this->_db->getErrorMsg());
            	$result = false;
