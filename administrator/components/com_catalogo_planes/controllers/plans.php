 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerPlans extends JController {
	...
	function display() {
		...
		$opt = JHTML::_('select.option', '', ' - ' . JText::_('CP.PLANCATEGORY') . ' - ', 'category_id', 'category_name');
		// Se obtiene el modelo de la región
		$categoryModel =& $this->getModel('plancategory');
	...
	function edit() {
		...
		if ($id && empty($data->product_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=plans', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		...
		if ($data->country_id > 0) {
			...
			if ($params->cfg_use_regions == '1' && $data->region_id > 0) {
		...
		$taxes = $taxesModel->getList(true, array('published' => 1, 'product_type_code' => 'plans'));
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=plans';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=plans', $msg, $type);
	}
	...
	function orderdown() {
		...
		$model =& $this->getModel('plans');
		$model->move($cid, 1);
		$this->setRedirect('index.php?option=' . $option . '&view=plans');
	}
	...
	function access() {
		...
		$row =& JTable::getInstance('plans', 'Table');
	...
	function listrates() {
		...
		// Redireccionar
		$this->setRedirect('index.php?option=' . $option . '&view=planrate&product_id=' . $cid . '&season_year=' . date('Y'));
	}
	...
	function autoloadstock() {
		...
		if (count($params) < 1) {
			JError::raiseError(500, JText::_('CP.PLAN_ERROR_EMPTY_PARAM1'));
		}
