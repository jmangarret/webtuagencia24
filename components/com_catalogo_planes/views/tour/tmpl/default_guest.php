...
$document->addScript(JURI::base().'components/com_catalogo_planes/views/plan/tmpl/cal_plan.js');
$currentYear=date("Y");
$currentYear=$currentYear-$minAgeAdults;
$scriptGlobalMSG = "
var maxDate = '".$currentYear."';
";
$document->addScriptDeclaration ($scriptGlobalMSG);
...
<div class="conte_plan">
	...
	<div class="precio_tit_precio_plan_dispo">
		...
		<div class="center_dias_plan">
			...
			<?php echo $this->data["duration_text"]?>
			</div>
		</div>
	...
	<form
		action="<?php echo JRoute::_("index.php?option=com_catalogo_planes&view=tour&layout=booking")?>"
		...
		<div class="conte_general">
			...
			<div class="conte_guest">
				...
				<table>
					...
					<?php if($i<$this->data["guest"]["adults"]):?>
						...
						<td><select name="guest[<?php echo $i?>][treatment]"
							id="treatment" class="<?php echo $classRequired?>">
								...
								<option value="1"
								<?php echo($guest_before['treatment']==1 && $i==0)?'selected="selected"':''; ?>>
									<?php echo JText::_("CP.GUEST.TREATMENT.MR")?>
								</option>
								<option value="2"
								<?php echo($guest_before['treatment']==2 && $i==0)?'selected="selected"':''; ?>>
									<?php echo JText::_("CP.GUEST.TREATMENT.MRS")?>
								</option>
								<option value="3"
								<?php echo($guest_before['treatment']==3 && $i==0)?'selected="selected"':''; ?>>
					...
					<tr>
						...
						<td><input maxlength="60" type="text"
							name="guest[<?php echo $i?>][name]"
							value="<?php echo($guest_before['name'] && $i==0)?$guest_before["name"]:''; ?>"
							class="required" />
						</td>
						...
						<td><input maxlength="60" type="text"
							name="guest[<?php echo $i?>][lastname]"
							value="<?php echo($guest_before['lastname'] && $i==0)?$guest_before["lastname"]:''; ?>"
							class="required" />
						</td>
					</tr>
					<tr>
						...
						<td><select name="guest[<?php echo $i?>][documenttype]"
							id="documenttype" class="<?php echo $classRequired?>">
								...
								<option value="1"
								<?php echo($guest_before['documenttype']==1 && $i==0)?'selected="selected"':''; ?>>
									<?php echo JText::_("CP.GUEST.DOCUMENTTYPE.ID")?>
								</option>
								<option value="2"
								<?php echo($guest_before['documenttype']==2 && $i==0)?'selected="selected"':''; ?>>
									<?php echo JText::_("CP.GUEST.DOCUMENTTYPE.EXT")?>
								</option>
								<option value="3"
								<?php echo($guest_before['documenttype']==3 && $i==0)?'selected="selected"':''; ?>>
						...
						<td><input maxlength="25" type="text"
							name="guest[<?php echo $i?>][document]"
							value="<?php echo($guest_before['document'] && $i==0)?$guest_before["document"]:''; ?>"
					...
					<tr>
						...
						<td><input name="guest[<?php echo $i?>][nationality]" type="text"
							...
							value="<?php echo($guest_before['nationality'] && $i==0)?$guest_before["nationality"]:''; ?>" />
						</td>
						<?php if($i<$this->data["guest"]["adults"]):?>
						...
						<input type="text" name="guest[<?php echo $i?>][birthdate]" class="required" size="10" readOnly/>
							...
							<?php for($day=1;$day<=31;$day++):?>
								...
								<?php echo($guest_before['bday']==$day && $i==0)?'selected="selected"':'' ?>>
									<?php echo $day?>
								</option>
								<?php endfor;?>
							...
							id="bMonth" class="required">
								<option value="1"
								<?php echo($guest_before['bmonth']==1 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.JANUARY.SHORT.LABEL'); ?>
								</option>
								<option value="2"
								<?php echo($guest_before['bmonth']==2 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.FEBRUARY.SHORT.LABEL'); ?>
								</option>
								<option value="3"
								<?php echo($guest_before['bmonth']==3 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.MARCH.SHORT.LABEL'); ?>
								</option>
								<option value="4"
								<?php echo($guest_before['bmonth']==4 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.APRIL.SHORT.LABEL'); ?>
								</option>
								<option value="5"
								<?php echo($guest_before['bmonth']==5 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.MAY.SHORT.LABEL'); ?>
								</option>
								<option value="6"
								<?php echo($guest_before['bmonth']==6 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.JUNE.SHORT.LABEL'); ?>
								</option>
								<option value="7"
								<?php echo($guest_before['bmonth']==7 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.JULY.SHORT.LABEL'); ?>
								</option>
								<option value="8"
								<?php echo($guest_before['bmonth']==8 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.AUGUST.SHORT.LABEL'); ?>
								</option>
								<option value="9"
								<?php echo($guest_before['bmonth']==9 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.SEPTEMBER.SHORT.LABEL'); ?>
								</option>
								<option value="10"
								<?php echo($guest_before['bmonth']==10 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.OCTOBER.SHORT.LABEL'); ?>
								</option>
								<option value="11"
								<?php echo($guest_before['bmonth']==11 && $i==0)?'selected="selected"':'' ?>>
									<?php echo JText::_('CP.MONTHS.NOVEMBER.SHORT.LABEL'); ?>
								</option>
								<option value="12"
								<?php echo($guest_before['bmonth']==12 && $i==0)?'selected="selected"':'' ?>>
						...
						$anos = 100;?> <select name="guest[<?php echo $i?>][byear]"
							...
							<?php for($y=$currentYear;$y>=($currentYear-$anos);$y--):?>
								...
								<?php echo($guest_before['byear']==$y && $i==0)?'selected="selected"':'' ?>>
									<?php echo $y?>
								</option>
								<?php endfor;?>
						</select>
			...
			<div class="conte_guest">
				...
				<table width="100%" cellspacing="3" cellpadding="2" border="0">
					...
					<td>
					<textArea name="contact[observation]" maxlength="256" class="campo_form_reserva" rows="4"></textArea>
					</td>
					</tr>*/ ?>
				</table>
			</div>
			<div class="conte_payment">
				...
				<?php if($this->data["haveStock"]):?>
					...
					<input type="radio" name="payment" id="pending" value="agency" /> <label
						...
						<?php /*<input type="radio" name="payment" id="destiny" value="destiny"/>
						<label for="destiny"><?php echo JText::_("CP.GUEST.PAYMENT.DESTINY")?></label> */?>
				</div>
				<div>
					<div class="conte_condition">
						<input type="checkbox" name="conditions_check"
							...
							id="button_next"> <?php echo JText::_("CP.TOUR.BUTTON.DETAIL.SELECT")?>
...
<div class="conte_tarif">
	...
	<?php echo JText::_("CP.BOOKING.INFO")?>
		<a target="_BLANK"
			href="<?php echo JRoute::_("index.php?option=com_catalogo_planes&view=tour&layout=pdfdetail&format=pdf");?>">
			<img alt="PDF"
	...
	<div class="detalles_precio_plan">
		<h3 class="seccion_title">
			<img
				...
				<?php echo JText::_("CP.TOUR.DATES.TITLE"); ?>
		</h3>
		<div class="conte_resume">
			<div>
				<strong><?php echo JText::_("CP.TOUR.DATE.CHECKIN").": "?> </strong>
				<?php echo $this->data["date"]["checkin"]?>
			</div>
			<div>
				<strong><?php echo JText::_("CP.TOUR.NUMBER.NIGHTS").": "?> </strong>
				<?php echo $this->data["duration_text"]?>
			</div>
		</div>
		<h3 class="seccion_title">
			<img
				...
				<?php echo JText::_("CP.TOUR.GUESTS.TITLE");?>
		</h3>
		<div class="conte_resume">
		<?php if($this->data["guest"]["adults"]>0):?>
			<div>
				<strong><?php echo JText::_("CP.TOUR.ADULTS").": "?> </strong>
				<?php echo $this->data["guest"]["adults"]?>
			</div>
			<?php endif;?>
			<?php if($this->data["guest"]["childs"]>0):?>
			<div>
				<strong><?php echo JText::_("CP.TOUR.CHILDS").": "?> </strong>
		...
		<div class="conte_resume">
			<table class="info_tarifas">
				<tr>
					<th><?php echo JText::_("CP.TOUR.GUEST.ROOM.QUANTITY")?></th>
					<th><?php echo JText::_("CP.TOUR.GUEST.ROOM.SERVICE")?></th>
					<th><?php echo JText::_("CP.TOUR.GUEST.ROOM.RATE")?></th>
				</tr>
				...
				<tr>
					...
					<?php echo JText::_("CP.TOUR.SUBTOTAL")?>
						</div></td>
					<td align="left"><?php echo $this->getFormatPrice($this->data["subtotal"],$this->data["currency"]["symbol"])?>
					</td>
				</tr>
				<tr>
					<td></td>
					<td><div class="subtotal">
					<?php echo JText::_("CP.TOUR.TAXES")?>
						</div></td>
...
<div id="dialog_mail" style="display: none;">
	<label><?php echo JText::_("CP.GUEST.TO")?> </label> <input type="text"
		maxlength="60" name="toMail" id="to_mail"
