...
/**
 * transferparam2 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTransferParam2 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('transferparam2');
		$view = & $this->getView('transferparam2', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param2_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=transferparam2', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		$editable = true;
		...
		if ($id) {
			...
			// No modificable si tiene productos asignados
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=transferparam2';
		if ($this->task == 'apply') {
			$param2_id = JRequest::getInt('param2_id');
			if ($param2_id < 1) {
				$data =& $model->getRow();
				$param2_id = $data->param2_id;
			}
			$link .= '&task=edit&cid[]=' . $param2_id;
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=transferparam2', $msg, $type);
	}
