...
class CatalogoPlanesViewTransfers extends JView {
	...
	function display ($tpl = null) {
		...
		switch ($layout) {
			case 'form':
                ...
                $title .= ' ' . JText::_('CP.TRANSFER');
                // Se colocan los botones en la barra de herramientas
                if (!$isNew) {
                    $bar = & JToolBar::getInstance('toolbar');
                    $bar->appendButton('Link', 'stock', JText::_('CP.STOCK'), 'index.php?option=' . $option . '&view=transfers&task=showstock&cid[]=' . $data->product_id);
                    $bar->appendButton('Link', 'rates', JText::_('CP.RATES'), 'index.php?option=' . $option . '&view=transferrate&task=listrates&product_id=' . $data->product_id);
                ...
                $form = &JForm::getInstance('',JPATH_COMPONENT.DS.'models'.DS.'transfers.xml');
                ...
                print_r( implode(',', $categories));
                $form->setValue('productid','details', $data->product_id);
                $form->setValue('categories','details', implode(',', $categories));
                ...
                $form->setValue('transfer_type','details', $data->transfer_type);
            ...
            case 'default_stock':
                $data = $this->get('data');
                // Obtiene título de la interfaz
                $title = JText::_('CP.STOCK') . ' :: <a href="index.php?option=' . $option . '&view=transfers&task=edit&cid[]=' . $data->product_id . '" class="product_link">' . $this->product_name . '</a>';
                $icon = 'stock.png';
			...
			default:
	            ...
	            JSubMenuHelper::addEntry(JText::_('CP.TRANSFERS'), 'index.php?option=' . $option . '&view=transfers');
	            JSubMenuHelper::addEntry(JText::_('CP.TRANSFERCATEGORY'), 'index.php?option=' . $option . '&view=transfercategory');
	            JSubMenuHelper::addEntry(JText::_('CP.TRANSFERPARAM1'), 'index.php?option=' . $option . '&view=transferparam1');
	            JSubMenuHelper::addEntry(JText::_('CP.TRANSFERPARAM2'), 'index.php?option=' . $option . '&view=transferparam2');
	            JSubMenuHelper::addEntry(JText::_('CP.TRANSFERPARAM3'), 'index.php?option=' . $option . '&view=transferparam3');

				// Obtiene título de la interfaz
				$title = JText::_('CP.CATALOGOPLANES') . ' :: ' . JText::_('CP.TRANSFERS');
				$icon = 'transfer.png';
				break;
		}
