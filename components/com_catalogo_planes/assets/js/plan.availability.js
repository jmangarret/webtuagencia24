...
function setDefaults(currentFilter){
	...
	if(isFilterAccomodationType && currentFilter!="producttype"){
		jQuery( "."+elementProductType).attr("checked",true);
 ...
 * @param classElement
 */
function setFilterAccomodation(classElement, parameterElement){
	isFilterAccomodationType=true;
	if(parameterElement=='accomodation')
		elementAccomodation = classElement;
	else
		elementProductType = classElement;
	jQuery("."+classElement).click(function(){
		if(jQuery("."+classElement+":checked").length>0){
			
			
				filterAccomodation(classElement,parameterElement);
 ...
 * @param classElement
 */
function filterAccomodation(classElement,parameterElement){
	...
	jQuery("."+classElement).each(function(index,value){	
		...
		if(jQuery(this).attr("checked")){
			jQuery(".fila_resultado_planes["+parameterElement+"='"+jQuery(this).val()+"']").attr("filter","");
			jQuery(".fila_resultado_planes["+parameterElement+"='"+jQuery(this).val()+"']").css("display","block");
		}else{
			jQuery(".fila_resultado_planes["+parameterElement+"='"+jQuery(this).val()+"']").attr("filter","accomodation");
			jQuery(".fila_resultado_planes["+parameterElement+"='"+jQuery(this).val()+"']").css("display","none");
		}		
	...
	var classElement_strs = classElement.split("_")

	setDefaults(classElement_strs[1]);
}
function filterAccomodationTour(classElement){

	jQuery(classElement).each(function(index,value){	
	

		if(jQuery(this).attr("checked")){
			jQuery(".fila_resultado_planes[product_type='tour']").attr("filter","");
			jQuery(".fila_resultado_planes[product_type='tour']").css("display","block");
		}else{
			jQuery(".fila_resultado_planes[product_type='tour']").attr("filter","accomodation");
			jQuery(".fila_resultado_planes[product_type='tour']").css("display","none");
		}		
