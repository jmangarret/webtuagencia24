 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotelAmenity extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('hotelamenity');
		$view = & $this->getView('hotelamenity', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->amenity_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotelamenity', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotelamenity';
		if ($this->task == 'apply') {
			$amenity_id = JRequest::getInt('amenity_id');
			if ($amenity_id < 1) {
				$data =& $model->getRow();
				$amenity_id = $data->amenity_id;
			}
			$link .= '&task=edit&cid[]=' . $amenity_id;
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=hotelamenity', $msg, $type);
	}
