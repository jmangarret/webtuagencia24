 ...
 *
 */
class Tour extends Products{

  private $codeAgency;
  private $productName = "tour";



  /**
   *
   * This function return the details of the tour
   * @param $requestXML
   * @param $xmlHeader
   */
  function details($requestXML, $xmlHeader){
    ...
    $sql = "CALL DetailsTour(".$prodictId.", '".$requestXML->language."')";
  ...
  private function objectToXmlDetails($xmlHeader, $object, $productId){
    ...
    $langArray = $this->getLangObjectByTableAndId("cp_tours_info", $productId, $xmlHeader->language);
    //Get the image list
    $imageArray = $this->getImageList($productId, $this->productName);/*mod proce*/
    //get the amenity list
    $amenityArray = $this->getAmenities($xmlHeader->language, $productId);
    //Get the image list
    $commentArray = $this->getComment($productId, $this->productName);/*mod proce*/
    //Get the seasons list
    $seasonsArray = $this->getSeasons($productId, $this->productName, $xmlHeader->language);/*mod proce*/
    //Get the param1 list
    $param1Array = $this->getParam1($productId, $this->productName, $xmlHeader->language);/*mod proce*/
    //Get the param2 list
    $param2Array = $this->getParam2($productId, $this->productName, $xmlHeader->language);/*mod proce*/
    //Get the param3 list
    $param3Array = $this->getParam3($productId, $this->productName, $xmlHeader->language);
    //Get the supplement list
    $supplementArray = $this->getSupplements($productId, $this->productName, $xmlHeader->language);/*mod proce*/
    //get the tourismtype list
    $tourismTypeArray = $this->getTourismType($xmlHeader->language, $this->productName);/*mod proce*/
    //GEt the supplier data
    $supplierData = $this->getProductSupplier($object->supplier_id, $xmlHeader->language);
    //get the category list
    $categoryArray = $this->getcategory($xmlHeader->language, $this->productName);/*mod proce*/
    ...
    $xmlHeader->data->addChild("adultwithoutchild", $object->adult_without_child);
    $xmlHeader->data->addChild("param2Adults", "1");
    $xmlHeader->data->addChild("param2Childs", "2");
  ...
  function rates($requestXML, $xmlHeader){
    ...
    $rateSupplement = $this->getSupplementTour($requestXML, $numberWeek);
    //Obtiene el xml organizado para enviarlo a la peticion
  ...
  function objectToXmlRate($xmlHeader, $object, $rateSupplement, $productId, $checkin_date, $currencyId){
    ...
    if(count($arraySupplementsType2)>0){
      foreach ($arraySupplementsType2 as $supplementId=>$price){
        ...
        $arrayTaxSupplement = $this->getSupplementTaxes($this->productName, $xmlHeader->language, $productId,$supplementId );//modificar sp
  ...
  private function getSupplementTour($requestXML, $dayOfWeek){
    //Instancia la conexion
    $objConection = new DataConection();
    //Llama al procedimiento almacenado enviandole los parametros
    $sql = "CALL GetSupplementRateTour(
			".$this->serviceConfig->applyMarkup.",
			".$requestXML->product_id.",
			'".$requestXML->checkin_date."',
			".$dayOfWeek.",
