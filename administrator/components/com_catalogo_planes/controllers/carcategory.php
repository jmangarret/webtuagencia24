 ...
 *
 * @version $Id: carcategory.php 2012-09-11 8:29:50 svn $
 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerCarCategory extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('carcategory');
		$view = & $this->getView('carcategory', 'html');
	...
	function edit() {
		...
		if ($cid[0] && empty($data->category_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=carcategory', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=carcategory';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=carcategory', $msg, $type);
	}
