...
Joomla.submitbutton = function(pressbutton) {
        ...
        if (task == "manualloadstock") {
	        ...
	        if (jQuery('select[name^="param_id"] option:selected').length < 1) {
	            submit = false;
	            alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_PARAM1', true); ?>");
	        } else if (value.length < 1 || exp.test(value)) {
	            submit = false;
	            form.quantity.focus();
	            alert("<?php echo JText::_('CP.PRODUCT_STOCK_ERROR_INVALID_QUANTITY', true); ?>");
	        }
...
?>
	<table border="0" cellpadding="5" cellspacing="1">
		<tbody>
            <tr>
				<td rowspan="4">
				    <label for="param_id"><strong><?php echo JText::_('CP.PLANPARAM1_LABEL'); ?></strong></label><br />
				    <?php echo JHTML::_('select.genericlist', $data->stockParams, 'param_id[]', 'class="inputbox" multiple="multiple" size="10"', 'param_id', 'param_name'); ?>
				 </td>
			</tr>
            <tr>
