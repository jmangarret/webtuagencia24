<?php
    ...
    //RURIEPE 24/08/2016 - ARRAY PRA CARGAR SELECT CON LOS VALORES PARA LOS DESTINOS
    $data = array(
 
        (object)array("titulo"=>"Buenos Aires"),
        (object)array("titulo"=>"La Habana"),
        (object)array("titulo"=>"Madrid"),
        (object)array("titulo"=>"New York"),
        (object)array("titulo"=>"Quito"),
        (object)array("titulo"=>"Santiago de Chile"),
        (object)array("titulo"=>"Sao Paulo"),
        (object)array("titulo"=>"Otro"),
...
<link rel="stylesheet" href="<?php echo JURI::root(); ?>modules/mod_jdcontact/tmpl/lib/contact.css" media="screen" />
    ...
    <script type="text/javascript">
       ...
       //RURIEPE 22/08/2016 - CONDICION QUE EVALUA LA VARIABLE F_SALIDA PARA OCULTAR O NO LAS FECHAS Y ORIGEN-DESTINO, DEPENDIENDO DEL VALOR QUE TOME LA VARIABLE(SI ES IGUAL A defaultValue ESO INDICA QUE LA VARIABLE NO RECIBE NINGUN VALOR, A LO QUE SE REFIERE QUE SE DEBE ACTIVAR EL POP UP PARA EL BANNER DE DESTINOS SOTOS)
        if(f_salida != 'defaultValue')
        {
            $('#fechas').hide();
            $('#origen-destino').hide();
            $('#textarea-banner').hide()
        } 
        else
        {
            $('#textarea-search').hide();
        }
        ...
        //RURIEPE 22/08/2016 - SE REMUEVE EL VALOR QUE QUEDA ALMACENADO EN LAS VARIABLES
        localStorage.removeItem("c_origen");
        localStorage.removeItem("c_destino");
        localStorage.removeItem("f_salida");
        localStorage.removeItem("f_llegada");
        localStorage.removeItem("c_viaje");
        localStorage.removeItem("aerolinea");
        localStorage.removeItem("adultos");
        localStorage.removeItem("mayores");
        localStorage.removeItem("niños");
        localStorage.removeItem("bebés");
    </script> 
...
<div id="contactform" style="background: <?php echo $backgroundcolor; ?>;width: <?php echo $wrp_width; ?>;border:1px solid <?php echo $inputfield_border; ?>;">
    ...
    <span id="closedailyp" style="cursor: pointer; position: relative; bottom: 65px; margin-left: 307px; font-weight: bold;">X</span>

    <div style="background-color: #1467b6; margin-left: -21px;   margin-top: -100px; width: <?php echo $wrp_width; ?>; border-top-left-radius: 1em 1em; border-top-right-radius: 1em 1em;"><br>
        <h3 style="color: #FFFFFF" align="center">Hemos registrado su solicitud.</h3> 
        <p style="color: #FFFFFF; font-weight: bold; text-align: center;">Envíenos su información personal y seguro le contactaremos</p>
    </div>
    ...
    <form name="contactform" id="form" method="post" action="<?php $_SERVER['PHP_SELF']?>">
        ...
        <label class="name" style="color: #000000;"><?php echo "*Nombre";//echo JText::_('MOD_JDCONTACT_NAME'); ?><br/>
            <input style="background-color: white; width: <?php echo $inputfield_width; ?>;" class="text" name="name" type="text" value="<?php echo $name; ?>" /><br/>
        </label>

        <label class="email" style="color: #000000;"><?php echo"*Correo electrónico";//echo JText::_('MOD_JDCONTACT_EMAIL'); ?><br/>
            <input style="background-color: white; width: <?php echo $inputfield_width; ?>;" class="text" name="email" type="text" value="<?php echo $email; ?>" /><br/>
        </label>

        <label class="phno" style="color: #000000;"><?php echo"*Teléfono";//echo JText::_('MOD_JDCONTACT_TELEPHONE'); ?><br/>
            <input style="background-color: white; width: <?php echo $inputfield_width; ?>;" class="text" name="phno" type="text" value="<?php echo $phno; ?>" onkeypress="return justNumbers(event);" /><br/>
        </label>

        <!--RURIEPE 24/08/2016 - CAMPO DESTINO,LLENADO CON UN ARRAY, PPARA SER SELECCIONADO POR EL USUARIO Y EL CAMPO ORIGEN TIENE UN VALOR ASIGNADO QUE ES CARACAS - MAIQUETIA -->
        <div id="origen-destino">
            <label class="origen" style="color: #000000;float: left;"><?php echo "*Origen";//echo JText::_('MOD_JDCONTACT_NAME'); ?><br/>
                <input style="background-color: white; width:145px; " class="text" name="origen" type="text" value="Caracas - Maíquetia (Apto. Int. Simón Bolívar)" readonly />&nbsp;&nbsp;&nbsp;<br/>
            </label>

            <label class="destino" style="color: #000000;"><?php echo "*Destino";//echo JText::_('MOD_JDCONTACT_NAME'); ?><br/>
                <select style="background-color: white; width:145px;" class="text" name="destino" type="text"/>
                    <option selected="selected" value="0">--Seleccione--</option>
                    <?php 
                        for($i=0; $i<count($data); $i++)
                        {
                    ?>
                            <option value="<?php echo $data[$i]->titulo ?>"><?php echo $data[$i]->titulo ?></option>
                    <?php
                        }
                    ?>
                </select><br/>
        ...
        <!--RURIEPE 22/08/2016 - CAMPOS FECHA DE SALIDA Y LLEGADA -->
        <div id="fechas">
            <label class="f_salida_banner" style="color: #000000;float: left;"><?php echo "*Fecha de salida";//echo JText::_('MOD_JDCONTACT_NAME'); ?><br/>
                <input style="background-color: white; width:145px; " class="text" name="f_salida_banner" type="date"/>&nbsp;&nbsp;&nbsp;<br/>
            </label>

            <label class="f_llegada_banner" style="color: #000000;"><?php echo "*Fecha de regreso";//echo JText::_('MOD_JDCONTACT_NAME'); ?><br/>
                <input style="background-color: white; width:145px;" class="text" name="f_llegada_banner" type="date" /><br/>
            </label>
        </div>

        <div id="textarea-search">

            <label class="msg" style="color: #000000;"><?php echo"Información Adicional (Opcional)";//echo JText::_('MOD_JDCONTACT_MESSAGE'); ?><br/>
                <textarea style="background-color: white; width: <?php echo $inputfield_width; ?>; " class="text" name="msg"><?php echo $msg; ?></textarea><br/>
            </label>
        </div>
        <div id="textarea-banner">

            <label class="msg_banner" style="color: #000000;"><?php echo"Información Adicional (Opcional)";//echo JText::_('MOD_JDCONTACT_MESSAGE'); ?><br/>
                <textarea style="background-color: white; height:40px; width: <?php echo $inputfield_width; ?>; " class="text" name="msg_banner"><?php echo $msg; ?></textarea><br/>
            </label>
         </div>

        <label style="color: #000000; font-weight: bold;">*Campos Obligatorios.</label></br>
        <center>
            <span style="font-size: 11pt; color: #21A73C;" ><img src="<?php echo JURI::root(); ?>images/img/ic-whatsapp.png" height="20px;" alt="ic-line" border="0">WhatsApp</span>
            <span class="big-blue" style="font-weight: bold; font-size: 11pt;">(0426) 401 2821</span>&nbsp;
        </center>
        <hr>
        ...
        $("#submit").click(function()
        {
            ...
            $.ajax(
            {
                ...
                success: function(msg)
                {
                    ...
                    if (msg=='<?php echo "<label style=color:#1467b6>Datos enviados correctamente,Nos contactaremos con usted cuando dispongamos de un vuelo a su destino.</label>"; ?>') 
                    {
                        $('#submit').hide(700);
                        $("#dailyfullscreen").delay(3000).animate(
                        { opacity: "toggle" }, 
                        function(){
                        $("#dailyfullscreen").remove(); 
