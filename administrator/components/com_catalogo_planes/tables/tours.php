...
* @subpackage       catalogo_planes
*/
class TableTours extends BasicTable {
    ...
    function __construct(& $db) {
        ...
        $this->relations['tourismtypes']['referencedtable'] = '#__cp_tours_tourismtype';
        ...
        $this->relations['mediafiles']['referencedtable'] = '#__cp_tours_files';
        ...
        $this->relations['rateparam1']['referencedtable'] = '#__cp_tours_param1';
        ...
        $this->relations['rateparam2']['referencedtable'] = '#__cp_tours_param2';
        ...
        $this->relations['taxes']['referencedtable'] = '#__cp_tours_taxes';
        ...
        $this->relations['supplements']['referencedtable'] = '#__cp_tours_supplement';
        ...
        $this->relations['supps_taxes123']['referencedtable'] = '#__cp_tours_supplement_tax';
        ...
        $this->relations['resume']['referencedtable'] = '#__cp_tours_resume';
        ...
        $this->relations['stock']['referencedtable'] = '#__cp_tours_stock';
        ...
        $this->relations['comments']['referencedtable'] = '#__cp_tours_comments';
        ...
        parent::__construct('#__cp_tours_info', 'product_id', $db);
    }
    ...
    function check() {
        ...
        if (!$this->duration) {
            $this->setError(JText::_('CP.TOUR.PRODUCT_ERROR_EMPTY_DURATION'));
            return false;
        }
        ...
        if ($id) {
            $this->setError(JText::_('CP.TOUR_ERROR_DUPLICATE_NAME'));
            return false;
        }

        // Verifitour que no haya registros con el mismo código
        $query = 'SELECT ' . $this->_tbl_key . ' FROM ' . $this->_tbl;
        $query .= ' WHERE LOWER(product_code) = ' . $this->_db->Quote(strtolower($this->product_code)) . ' AND product_id != ' . (int) $this->product_id;
        $this->_db->setQuery($query);
        $id = $this->_db->loadResult();
        if ($id) {
        	$this->setError(JText::_('CP.TOUR_ERROR_DUPLICATE_CODE'));
        	return false;
        }
    ...
    function duplicate($cid) {
        if ($cid > 0) {
            ...
            $query = 'SELECT CLONETOUR(' . $cid . ', \'' . JText::_('CP.PRODUCT_DUPLICATE_NAME_PREFIX') . '\', ' . $user->get('id') . ', \''.date("mdHis").'\')';
            $this->_db->setQuery($query);
    ...
    function store($updateNulls = false, $updateRelationships = true) {
        ...
        if ($result && $id) {
            ...
            $query = 'DELETE FROM #__cp_tours_supplement_tax WHERE product_id = ' . $id;
            $this->_db->setQuery($query);
            ...
            if ($result && !empty($this->supplement_taxes)) {
                ...
                $query = 'INSERT INTO #__cp_tours_supplement_tax (product_id, supplement_id, tax_id) VALUES ' . implode(', ', $insertSupplements);
                $this->_db->setQuery($query);
            ...
            if ($result) {
                $calculatePrices = 0;
                $query = 'CALL ValidateTourRates(' . $id . ', ' . $calculatePrices . ')';
                $this->_db->setQuery($query);

                if (!$this->_db->query()) {
                    $this->setError($this->_db->getErrorMsg());
                    $result = false;
                    $query = 'UPDATE #__cp_tours_info SET published = 0 WHERE product_id = ' . $id;
                    $this->_db->setQuery($query);
                    $this->_db->query();
                }
            }
