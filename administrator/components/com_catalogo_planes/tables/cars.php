 ...
 *
 * @version $Id: cars.php 2012-09-11 8:29:50 svn $
...
* @subpackage       catalogo_planes
*/
class TableCars extends BasicTable {
    ...
    function __construct(& $db) {
        ...
        $this->relations['mediafiles']['referencedtable'] = '#__cp_cars_files';
        ...
        $this->relations['rateparam1']['referencedtable'] = '#__cp_cars_param1';
        ...
        $this->relations['rateparam2']['referencedtable'] = '#__cp_cars_param2';
        $this->relations['rateparam2']['referencedfield'] = 'product_id';
        $this->relations['rateparam2']['referencedtableid'] = 'param2_id';

        $this->relations['deliverycities'] = array();
        $this->relations['deliverycities']['referencedtable'] = '#__cp_cars_delivery_city';
        $this->relations['deliverycities']['referencedfield'] = 'product_id';
        $this->relations['deliverycities']['referencedtableid'] = 'city_id';
        $this->relations['deliverycities']['extrainfotable'] = '#__cp_prm_cars_delivery_city';
        $this->relations['deliverycities']['extrainfoid'] = 'city_id';
        $this->relations['deliverycities']['extrainfofields'] = array('city_id');
        $this->relations['deliverycities']['orderby'] = array('city_name');

        $this->relations['taxes'] = array();
        $this->relations['taxes']['referencedtable'] = '#__cp_cars_taxes';
        ...
        $this->relations['supplements']['referencedtable'] = '#__cp_cars_supplement';
        ...
        $this->relations['supps_taxes123']['referencedtable'] = '#__cp_cars_supplement_tax';
        ...
        $this->relations['resume']['referencedtable'] = '#__cp_cars_resume';
        ...
        $this->relations['stock']['referencedtable'] = '#__cp_cars_stock';
        ...
        $this->relations['comments']['referencedtable'] = '#__cp_cars_comments';
        ...
        parent::__construct('#__cp_cars_info', 'product_id', $db);
    }
    ...
    function check() {
        ...
        if ($id) {
            $this->setError(JText::_('CP.CAR_ERROR_DUPLICATE_NAME'));
            return false;
        }
        ...
        $id = $this->_db->loadResult();
        if ($id) {
        	$this->setError(JText::_('CP.CAR_ERROR_DUPLICATE_CODE'));
        	return false;
        }
    ...
    function duplicate($cid) {
        if ($cid > 0) {
            ...
            $query = 'SELECT CLONECAR(' . $cid . ', \'' . JText::_('CP.PRODUCT_DUPLICATE_NAME_PREFIX') . '\', ' . $user->get('id') . ', \''.date("mdHis").'\')';
            $this->_db->setQuery($query);
    ...
    function store($updateNulls = false, $updateRelationships = true) {
        ...
        if ($result && $id) {
            ...
            $query = 'DELETE FROM #__cp_cars_supplement_tax WHERE product_id = ' . $id;
            $this->_db->setQuery($query);
            ...
            if ($result && !empty($this->supplement_taxes)) {
                ...
                $query = 'INSERT INTO #__cp_cars_supplement_tax (product_id, supplement_id, tax_id) VALUES ' . implode(', ', $insertSupplements);
                $this->_db->setQuery($query);
            ...
            if ($result) {
                $calculatePrices = 0;
                $query = 'CALL ValidateCarRates(' . $id . ', ' . $calculatePrices . ')';
                $this->_db->setQuery($query);

                if (!$this->_db->query()) {
                    $this->setError($this->_db->getErrorMsg());
                    $result = false;
                    $query = 'UPDATE #__cp_cars_info SET published = 0 WHERE product_id = ' . $id;
                    $this->_db->setQuery($query);
                    $this->_db->query();
                }
            }
