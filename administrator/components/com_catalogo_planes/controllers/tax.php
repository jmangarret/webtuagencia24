 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTax extends JController {
	...
	function edit() {
		...
		if ($id && empty($data->tax_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=tax', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=tax';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=tax', $msg, $type);
	}
