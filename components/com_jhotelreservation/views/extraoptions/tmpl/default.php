		...
		<div CLASS='div-buttons'>
			<table width='100%' border= 0 align=center>
				<tr>
					...
					<td align=right>
						<button class="ui-hotel-button" name="checkRates" type="button" onclick="return checkContinue();">
							<span class="ui-button-text">
								<?php echo $this->userData->rooms > count($this->userData->reservedItems) ?	JText::_('LNG_CONTINUE_ROOM_RATES')	: JText::_('LNG_CONTINUE')?>
							</span>
						</button>
					</td>
				</tr>
		...
		?>
			...
			?> 
				<fieldset>
					...
					<div id="extra-options-container" style="float: left;">
						<?php
									$cadena = array(
									    0  => "Pasaje Aereo",
									    1  => "pasaje aereo",
									...
									foreach ($this->extraOptions as $extra) {
										...
										$coincidencia = strpos($extra->name, $cadena[$i]);
										if(is_int($coincidencia)){
											$encontrado = true;
											//$con = $coincidencia;
										}
										//echo $cadena[$i]."\n";
								...
								<div class="extra-option">
									...
									<div class="extras-options" id="options-<?php echo $item->id?>" style="display:none">
										<?php if($item->price_type == 1){ ?>
											...
											?>
											<br>Número de Niños
											<select id="niños-extras-<?php echo $item->id?>" name="niños-extras-<?php echo $item->id?>" onchange="upateExtraOption()">
												<?php for($i=0;$i<6;$i++){ ?>
													<option value="<?php echo $i ?>" <?php echo $item->mandatory == 1 && $i== ($this->_models['variables']->guest_adult + $this->_models['variables']->guest_child)? 'selected="selected"':''?>><?php echo $i ?></option>
												<?php 
												}
...
//dmp($grupos);
				...
				<center>
				<hr>
				<label><strong>Itinerario de vuelo:</strong></label><br>
				<textarea rows="6" cols="40" id="itinerario2" name="extra-option-itinerary"></textarea><br>
				<!--<input type="button" id="cargar" value="cargar" name="cargar">-->
				</center>
				<?php
					}
				}
				...
				<script type="text/javascript">
					...
					$('select#niños-extras').on('change',function() {
				...
				<!--/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
				<!--MICOD
				Lugar donde estoy renderizando el Jreservationinfo (Información previa a la cotización)-->
				<div style="float: right; margin-top:-50px;">
				      <?php
				            ...
				            $module = JModuleHelper::getModule( 'mod_jreservationinfo'); //Llamar la función de renderizar módulos
