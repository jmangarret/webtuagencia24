 ...
 *
 * @version $Id: transfercategory.php 2012-09-11 8:29:50 svn $
...
/**
 * transfercategory Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTransferCategory extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('transfercategory');
		$view = & $this->getView('transfercategory', 'html');
	...
	function edit() {
		...
		if ($cid[0] && empty($data->category_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=transfercategory', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=transfercategory';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=transfercategory', $msg, $type);
	}
	...
	function publish() {
		...
		// Se obtiene el modelo
		$model = $this->getModel('transfercategory');
		// Se obtienen los ids de los registros a publitransfer
		$cids = JRequest::getVar('cid', array(0), 'post', 'array');
		JArrayHelper::toInteger($cids);

		// Lanzar error si no se ha seleccionado al menos un registro a publitransfer
		if (count($cids) < 1) {
	...
	function unpublish() {
		...
		// Lanzar error si no se ha seleccionado al menos un registro a despublitransfer
		if (count($cids) < 1) {
