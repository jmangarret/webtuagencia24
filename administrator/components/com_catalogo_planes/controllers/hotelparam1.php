 ...
 *
 * @version $Id: hotelparam1.php 2012-09-11 8:29:50 svn $
...
/**
 * hotelparam1 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotelParam1 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('hotelparam1');
		$view = & $this->getView('hotelparam1', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param1_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotelparam1', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotelparam1';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=hotelparam1', $msg, $type);
	}
