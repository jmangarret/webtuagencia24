gajQuery(document).ready(function() {
	...
	gajQuery('input[name=date_from]').change(function() {
		gajQuery(this).closest('.ganalytics-table').parent().find('.ga-chart').gaChart('refresh', {
			start: gajQuery(this).val()
		});
	});
	gajQuery('input[name=date_to]').change(function() {
		gajQuery(this).closest('.ganalytics-table').parent().find('.ga-chart').gaChart('refresh', {
	...
	gajQuery('.date-range-button').click(function(){
		...
		gajQuery(this).parent().parent().find('.ga-chart').gaChart('refresh', {
			dateRange: range
		});
	});
});
...
function showCharts(){
	gajQuery('.ga-chart').gaChart('refresh', {
		start: gajQuery('input[name=date_from]').val(),
		end: gajQuery('input[name=date_to]').val()
	});
}