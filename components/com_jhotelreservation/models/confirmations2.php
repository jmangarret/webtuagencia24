							...
							for( $m = 0; $m < $max_subsubsubrecords ; $m ++ )
							{
								...
								switch( $i )
								{
									...
									case 1:
										...
										//dump($data->itemRoomsCapacity);
										//dump($rowTableManage->room_id);
										if( 
									...
									case 3:
										...
										{
											$obj->room_discount_id			=	$obj_ids[$j_key]->daily[$k_key]['numbers'][$l_key]['discounts'][$m_key]->room_discount_id;
										...
										{
// 											dump($obj_ids[$j_key]->daily[$k_key]['numbers'][$l_key]['discounts']);
											$obj->offer_room_discount_id	=	$obj_ids[$j_key]->daily[$k_key]['numbers'][$l_key]['discounts'][$m_key]->offer_room_price_id;
									...
									case 4:
										...
										if( $rowTableManage->package_id + 0 == 0 )
											continue;
										//dump($rowTableManage->package_id);	
									...
									case 10:
										...
										//dump($data->payment_processor_sel_id);
												...
												foreach( $ps as $row )
												{
													if( $row->AMT == 0.00 )
													{
														...
														if( $row->payment_percent > 0 )
															$value = my_round($value*$row->payment_percent/100,2);
														else
															$value = my_round($row->payment_value,2);

														$row->AMT = $value;
													}
													$arr_answer= array(0, '');

													// dbg($data->is_enable_payment);
													// dbg($row);
													// exit;
