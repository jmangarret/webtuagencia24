...
class skrill{    
	...
	public function initialize($data){
		...
		$this->notifyUrl = $data->fields['notify_url'];
		$this->currency = $data->fields['currency'];
		
		$this->log = Logger::getInstance();
		$this->log->LogDebug("init skrill processor");
	...
	public function getHtmlFields() {
		/*$url ="";
		$url.="?processor=skrill";
		$url.="&pay_to_email=".$this->merchant_email."&language=EN&currency=EUR&amount=".$this->amount;
		$url.="&transaction_id=".$this->itemNumber;
		$url.="&amount=".$this->amount."&detail1_description=". $this->itemName;
		$url.="&detail1_text=".$this->itemName."&confirmation_note=". $this->itemName;
		$url.="&return_url_target=2&cancel_url_target=2";
		$url.="&firstname=".$this->userData->first_name;
		$url.="&lastname=".$this->userData->last_name;
		$url.="&address=".$this->userData->address;
		$url.="&phone_number=".$this->userData->phone;
		$url.="&postal_code=".$this->userData->postal_code;
		$url.="&city=".$this->userData->city;
		$url.="&return_url_text=".JText::_('LNG_CONTINUE_RESERVATION');
		$url.="&state=".$this->userData->state;
		$url.="&country=".$this->userData->country;
		$url.="&pay_from_email=".$this->userData->email;
		
		$url.="&return_url=".$this->notifyUrl;
		$url.="&cancel_url=".$this->cancelUrl;
		$url.="&status_url=".$this->status_url;
		*/
		...
		$html .= sprintf('<input type="hidden" name="currency" id="currency" value="%s">', $this->currency);
	...
	public function processTransaction($data)
    {
    	...
    	$this->userData =$data->reservationData->userData;
    	$this->cancelUrl = JRoute::_('index.php?option=com_jhotelreservation&task=hotels.searchHotels',false,-1);;
    	$this->status_url = JRoute::_('index.php?option=com_jhotelreservation&task=paymentoptions.processAutomaticResponse',false,-1);
    ...
    public function startSession(){
    	try{
    		...
    		$url.="?pay_to_email=".$this->merchant_email."&language=EN&prepare_only=1&currency=$this->currency";
    		$url.="&transaction_id=".$this->itemNumber;
