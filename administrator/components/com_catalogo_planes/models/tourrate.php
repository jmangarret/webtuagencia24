 ...
 *
 * @version $Id: tourrate.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelTourRate extends CatalogoPlanesModelBasic {
	...
	function __construct() {
		...
		$this->_filter_prefix = $option . '_tourrate';
    }
	...
	function &getRow($id = null, $loadRelationships = true) {
		if (empty($this->_data)) {
			...
			if ($id) {
                ...
                // Tourgar el nombre del usuario que creó la vigencia
                if ($row->created_by) {
    ...
    function checkSeasonOverlapping($product_id, $season_id, $rate_id) {
        ...
        $query = 'SELECT s1.season_name, s1.is_special, s1.day1, s1.day2, s1.day3, s1.day4, 
            ...
            INNER JOIN #__cp_tours_rate r ON s1.season_id = r.season_id 
        ...
        if (empty($list)) {
            ...
            // Para vigencias estándar, verifitour si hay coincidencia en los días
            if ($list[0]->is_special == 0) {
            	...
            	// Para vigencias especiales no es necesario verifitour días
            	$result = $list;
            }
