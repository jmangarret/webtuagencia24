...
/**
 * hotelparam3 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotelParam3 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('hotelparam3');
		$view = & $this->getView('hotelparam3', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param3_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotelparam3', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotelparam3';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=hotelparam3', $msg, $type);
	}
