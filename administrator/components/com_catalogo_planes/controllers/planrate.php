 ...
 *
 * @version $Id: planrate.php 2012-09-11 8:29:50 svn $
 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerPlanRate extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('planrate');
		$modelSeason =& $this->getModel('season');
		$view = & $this->getView('planrate', 'html');
		...
		$modelProduct =& $this->getModel('plans');
	...
	function edit() {
		...
		if ($rate_id && !$data->season_id) {
			$this->setRedirect('index.php?option=' . $option . '&view=planrate&product_id=' . $product_id, JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		...
		if ($rate_id > 0) {
			$seasons = $modelSeason->getSeasonsByProductType('plans', $data->season_id, $product_id, $rate_id);
			$data->dates = $modelSeason->getSeasonDates($data->season_id);
		} else {
			$seasons = $modelSeason->getSeasonsByProductType('plans', 0, $product_id);
			$data->dates = array();
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=planrate&product_id=' . $product_id;
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$product_id = JRequest::getInt('product_id');
		$this->setRedirect('index.php?option=' . $option . '&view=planrate&product_id=' . $product_id, $msg, $type);
	}
