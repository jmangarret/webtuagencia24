...
class TableConfirmations extends JTable
{
	...
	function getCubilisReservations($hotelId, $limit){
		...
		$query = "UPDATE `goedverblijf`.`jos_hotelreservation_confirmations` SET `cubilis_status`=0 WHERE `confirmation_id`='9957'";
		$db->setQuery($query);
		$db->query();
		
		$query = "select c.*, GROUP_CONCAT(eo.extra_option_name,'||',eo.extra_option_price,'||',eo.extra_option_price_type,'||',eo.extra_option_is_per_day,'||',eo.extra_option_persons,'||',eo.extra_option_days separator '#') as extra_option_details
				 from  #__hotelreservation_confirmations c
				 left join  #__hotelreservation_confirmations_extra_options eo	ON eo.confirmation_id = c.confirmation_id 
				 where c.cubilis_status <> ".CUBILIS_RESERVATION_SENT." and c.hotel_id = $hotelId group by c.confirmation_id
			";
