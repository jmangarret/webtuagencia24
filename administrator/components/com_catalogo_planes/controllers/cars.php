 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerCars extends JController {
	...
	function display() {
		...
		$opt = JHTML::_('select.option', '', ' - ' . JText::_('CP.CARCATEGORY') . ' - ', 'category_id', 'category_name');
		// Se obtiene el modelo de la región
		$categoryModel =& $this->getModel('carcategory');
	...
	function edit() {
		...
		$taxes = $taxesModel->getList(true, array('published' => 1, 'product_type_code' => 'cars'));
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=cars', $msg, $type);
	}
	...
	function orderdown() {
		...
		$model =& $this->getModel('cars');
		$model->move($cid, 1);
		$this->setRedirect('index.php?option=' . $option . '&view=cars');
	}
	...
	function listrates() {
		...
		// Redireccionar
		$this->setRedirect('index.php?option=' . $option . '&view=carrate&product_id=' . $cid . '&season_year=' . date('Y'));
	}
	...
	function autoloadstock() {
		...
		if (count($params) < 1) {
			JError::raiseError(500, JText::_('CP.CAR_ERROR_EMPTY_PARAM1'));
		}
