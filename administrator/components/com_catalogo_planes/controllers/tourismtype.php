...
/**
 * tourismtype Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTourismType extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('tourismtype');
		$view = & $this->getView('tourismtype', 'html');
	...
	function edit() {
		...
		if ($cid[0] && empty($data->tourismtype_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=tourismtype', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=tourismtype';
		if ($this->task == 'apply') {
			...
			$link .= '&task=edit&cid[]=' . $tourismtype_id;
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=tourismtype', $msg, $type);
	}
