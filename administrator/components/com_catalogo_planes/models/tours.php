 ...
 *
 * @version $Id: tours.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelTours extends CatalogoPlanesModelBasic {

    function __construct() {
        $option = JRequest::getCmd('option');

        $this->_filter_prefix = $option . '_tours';
    ...
    /**
     * Tourga la información de un registro.
     * @param int $id
     * @return JTable
     */
    public function &getRow($id = null, $loadRelationships = true) {
        if (empty($this->_data)) {
            ...
            // Si se pasó un id válido, tourga la información
            if ($id) {
                $this->_data->load($id, $loadRelationships);
                if ($this->_data->country_id) {
                    // Tourgar el nombre del país
    ...
    protected function _buildQuery($use_filter = true, $conditions = array()) {
        ...
        $query .= ' a JOIN #__cp_prm_tours_category e ON a.category_id = e.category_id ';
        ...
        $query .= ' LEFT JOIN #__cp_tours_tourismtype f ON a.product_id = f.product_id ';
    ...
    public function store() {
        ...
        $data['adult_without_child'] = $details['adult_without_child'];
        ...
        if (!$row->store()) {
            ...
            if ($product_id && isset($data['mediafiles']) && is_array($data['mediafiles'])) {
                ...
                $query = 'DELETE FROM #__cp_tours_files WHERE `file_url` NOT IN (' . $files . ') AND `product_id` = ' . $product_id;
                ...
                foreach ($mediafiles as $file) {
                    ...
                    $query = 'SELECT `file_url` FROM #__cp_tours_files WHERE `file_url` = \'' . $file . '\' AND `product_id` = ' . $product_id;
                    $this->_db->setQuery($query);
                    $this->_db->query();
                    if (!$this->_db->getNumRows()) {
                        $query = 'INSERT INTO #__cp_tours_files (`product_id`, `file_url`, `ordering`) VALUES (' . $product_id . ', \'' . $file . '\', ' . $i . ')';
                    } else {
                        $query = 'UPDATE #__cp_tours_files SET `ordering` = ' . $i . ' WHERE `file_url` = \'' . $file . '\' AND `product_id` = ' . $product_id;
                    }
                    $this->_db->setQuery($query);
                    if (!$this->_db->query()) {
                        $this->setError($this->_db->getErrorMsg());
                ...
                $query = 'DELETE FROM #__cp_tours_files WHERE product_id = ' . $product_id;
                $this->_db->setQuery($query);
                if (!$this->_db->query()) {
                    $this->setError($this->_db->getErrorMsg());
                    return false;
    ...
    public function getRelatedSupplements($product_id, $getTaxes = true) {
        if ($getTaxes) {
            $query = 'SELECT a.*, d.tax_id, d.tax_name, d.tax_value, d.published as tax_published 
                FROM #__cp_prm_supplement a INNER JOIN #__cp_tours_supplement b ON a.supplement_id 
                = b.supplement_id LEFT JOIN #__cp_tours_supplement_tax c ON a.supplement_id = c.supplement_id AND 
            ...
            $query = 'SELECT a.* FROM #__cp_prm_supplement a INNER JOIN #__cp_tours_supplement b 
    ...
    public function getRateParams($product_id) {
        ...
        // Parámetro 1
        $query = 'SELECT a.* FROM #__cp_prm_tours_param1 a INNER JOIN #__cp_tours_param1 b 
        ...
        // Parámetro 2
        $query = 'SELECT a.* FROM #__cp_prm_tours_param2 a INNER JOIN #__cp_tours_param2 b 
    ...
    public function getStockParams($product_id) {
        $list = array();
        $list[] = array('param_id' => 1, 'param_name' => JText::_('CP.TOUR'));
    ...
    public function getStock($product_id, $start_date, $end_date) {
        $list = array();
        $query = 'SELECT param_id, `day`, quantity FROM #__cp_tours_stock WHERE product_id = ' . $product_id .
    ...
    function saveAutomaticStock($product_id, $params, $start_date, $end_date, $quantity) {
        ...
        foreach ($params as $param_id) {
            ...
            $query .= "CALL CleanTourStock($product_id, $param_id, '$start_date', '$end_date');\n";
            // Sólo se almacenan valores mayor o igual a 1 y fechas no vencidas
            if ($quantity < 1) continue;
            $query .= "CALL LoadTourStock($product_id, $param_id, '$start_date', '$end_date', $quantity);\n";
        }
    ...
    function saveManualStock($product_id, $stock, $start_date, $end_date) {
        ...
        $query .= "CALL CleanTourStock($product_id, 0, '$start_date', '$end_date');\n";
        ...
        foreach ($stock as $param_id => $values) {
            foreach ($values as $date => $quantity) {
                ...
                $query .= "CALL LoadTourStock($product_id, $param_id, '$date', '$date', $quantity);\n";
            }
        }
        // Se finaliza la transacción
        $query .= "COMMIT;";
