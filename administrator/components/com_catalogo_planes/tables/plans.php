 ...
 *
 * @version $Id: plans.php 2012-09-11 8:29:50 svn $
...
* @subpackage		catalogo_planes
*/
class TablePlans extends BasicTable {
	...
	function __construct(& $db) {
        ...
        $this->relations['tourismtypes']['referencedtable'] = '#__cp_plans_tourismtype';
        ...
        $this->relations['mediafiles']['referencedtable'] = '#__cp_plans_files';
        ...
        $this->relations['rateparam1']['referencedtable'] = '#__cp_plans_param1';
        $this->relations['rateparam1']['referencedfield'] = 'product_id';
        $this->relations['rateparam1']['referencedtableid'] = 'param1_id';
        $this->relations['rateparam1']['extrainfotable'] = '#__cp_prm_plans_param1';
        $this->relations['rateparam1']['extrainfoid'] = 'param1_id';
        $this->relations['rateparam1']['extrainfofields'] = array('param1_id');
        $this->relations['rateparam1']['orderby'] = array('param1_name');

        $this->relations['rateparam2'] = array();
        $this->relations['rateparam2']['referencedtable'] = '#__cp_plans_param2';
        ...
        $this->relations['rateparam3']['referencedtable'] = '#__cp_plans_param3';
        ...
        $this->relations['taxes']['referencedtable'] = '#__cp_plans_taxes';
        ...
        $this->relations['supplements']['referencedtable'] = '#__cp_plans_supplement';
        ...
        $this->relations['supps_taxes123']['referencedtable'] = '#__cp_plans_supplement_tax';
        ...
        $this->relations['resume']['referencedtable'] = '#__cp_plans_resume';
        ...
        $this->relations['stock']['referencedtable'] = '#__cp_plans_stock';
        ...
        $this->relations['comments']['referencedtable'] = '#__cp_plans_comments';
        ...
        parent::__construct('#__cp_plans_info', 'product_id', $db);
	}
	...
	function check() {
        ...
        // Verificar que se de la duración del plan
        if (!$this->duration) {
            $this->setError(JText::_('CP.PLAN.PRODUCT_ERROR_EMPTY_DURATION'));
            return false;
        }

        // Verificar que se del numero de días del plan
        if (!$this->days_total) {
            $this->setError(JText::_('CP.PLAN_ERROR_EMPTY_DAYS_TOTAL'));
            return false;
        }
		...
		if ($id) {
			$this->setError(JText::_('CP.PLAN_ERROR_DUPLICATE_NAME'));
			return false;
		}
		...
		if ($this->product_code) {
			...
			$id = $this->_db->loadResult();
			if ($id) {
				$this->setError(JText::_('CP.PLAN_ERROR_DUPLICATE_CODE'));
				return false;
			}
		}
    ...
    function duplicate($cid) {
        if ($cid > 0) {
            ...
            $query = 'SELECT CLONEPLAN(' . $cid . ', \'' . JText::_('CP.PRODUCT_DUPLICATE_NAME_PREFIX') . '\', ' . $user->get('id') . ', \''.date("mdHis").'\')';
            $this->_db->setQuery($query);
    ...
    function store($updateNulls = false, $updateRelationships = true) {
        ...
        if ($result && $id) {
        	...
        	$query = 'DELETE FROM #__cp_plans_supplement_tax WHERE product_id = ' . $id;
        	$this->_db->setQuery($query);
        	...
        	if ($result && !empty($this->supplement_taxes)) {
                ...
                $query = 'INSERT INTO #__cp_plans_supplement_tax (product_id, supplement_id, tax_id) VALUES ' . implode(', ', $insertSupplements);
                $this->_db->setQuery($query);
        	...
        	if ($result) {
	            $calculatePrices = 0;
	            $query = 'CALL ValidatePlanRates(' . $id . ', ' . $calculatePrices . ')';
	            $this->_db->setQuery($query);

	            if (!$this->_db->query()) {
	            	$this->setError($this->_db->getErrorMsg());
	            	$result = false;
		            $query = 'UPDATE #__cp_plans_info SET published = 0 WHERE product_id = ' . $id;
		            $this->_db->setQuery($query);
		            $this->_db->query();
	            }
            }
