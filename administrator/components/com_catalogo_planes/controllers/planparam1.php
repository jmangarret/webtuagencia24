...
/**
 * planparam1 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerPlanParam1 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('planparam1');
		$view = & $this->getView('planparam1', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param1_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=planparam1', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	/**
	 * Validad nombre de hotel
	 * @return void
	 */
    function validateHotelName($param1_name){
		$model =& $this->getModel('planparam1');

		$data = $model->getListByName($param1_name);
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=planparam1';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=planparam1', $msg, $type);
	}
    ...
    /**
     * Obtiene listado de registros por nombre
     */
    function getRawListByName() {
        $mainframe =& JFactory::getApplication(); 
        $conditions = array();
        ...
        // Recupera los valores de los filtros
        $param1_name = JRequest::getVar('param1_name', '');
