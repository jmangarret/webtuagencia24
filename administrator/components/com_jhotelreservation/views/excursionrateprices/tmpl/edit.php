...
<form action="<?php echo JRoute::_('index.php?option=com_jhotelreservation&view=excursionrateprices&layout=edit&rate_id='.$this->state->get("filter.rate_id")); ?>" method="post" name="quickFilterFrm" id="quickFilterFrm">
	<table class="rate-quick-filter">
		...
		<input type="hidden" name="task" value="excursionrateprices.quickSetup" /> 
		<input type="hidden" name="rate_id" id="rate_id" value="<?php echo $this->state->get("filter.rate_id") ?>" /> 
		<?php echo JHTML::_( 'form.token' ); ?> 
</form>
<?php } ?>
<form action="<?php echo JRoute::_('index.php?option=com_jhotelreservation&view=excursionrateprices&layout=edit&rate_id='.$this->state->get("filter.rate_id")); ?>" method="post" name="adminForm" id="excursion-form">
		...
		<div id="rate-wrapper">
			...
			<div class="rates-header">
				...
				<span> <?php echo JText::_('LNG_PRICE_TYPE',true).": ".($this->rate->price_type==1?JText::_('LNG_PER_PERSON',true):JText::_('LNG_PER_EXCURSION',true)) ?></span><br/>
	...
	<script language="javascript" type="text/javascript">
		...
		Joomla.submitbutton = function(task, type)
		{
			...
			if (task == 'item.cancel' || validateForm() ) {
				Joomla.submitform(task, document.id('excursion-form'));
			}
		}	
