 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTourRate extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('tourrate');
		$modelSeason =& $this->getModel('season');
		$view = & $this->getView('tourrate', 'html');
		...
		$modelProduct =& $this->getModel('tours');
	...
	function edit() {
		...
		if ($rate_id && !$data->season_id) {
			$this->setRedirect('index.php?option=' . $option . '&view=tourrate&product_id=' . $product_id, JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		...
		if ($rate_id > 0) {
			$seasons = $modelSeason->getSeasonsByProductType('tours', $data->season_id, $product_id, $rate_id);
			$data->dates = $modelSeason->getSeasonDates($data->season_id);
		} else {
			$seasons = $modelSeason->getSeasonsByProductType('tours', 0, $product_id);
			$data->dates = array();
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=tourrate&product_id=' . $product_id;
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$product_id = JRequest::getInt('product_id');
		$this->setRedirect('index.php?option=' . $option . '&view=tourrate&product_id=' . $product_id, $msg, $type);
	}
