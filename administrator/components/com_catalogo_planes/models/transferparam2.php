 ...
 *
 * @version $Id: transferparam2.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelTransferParam2 extends CatalogoPlanesModelBasic {
	...
	function __construct() {
		...
		$this->_filter_prefix = $option . '_transferparam2';
    }
	...
	protected function _buildQuery($use_filter = true, $conditions = array()) {
        ...
        $orderby    = ($use_filter && (count($conditions) < 1))? $this->_buildQueryOrderBy(): ' ORDER BY param2_name ASC';
	...
	public function getOrderByField() {
		...
		$filter_order = $mainframe->getUserStateFromRequest($this->_filter_prefix . 'filter_order', 'filter_order', 'param2_name', 'cmd');

		// se asegura que el orden esté dando por los campos aprobados
		if (!in_array($filter_order, array('param2_id', 'param2_name', 'capacity', 'value', 'published'))) {
			$filter_order = 'param2_name';
		}
	...
	protected function _buildQueryWhere($conditions = array()) {
        ...
        if (count($conditions) > 0) {
            ...
            $allowed_filters['param2_id'] = '`param2_id`';
			...
			if ($search) {
				$where[] = ' LOWER(`param2_name`) LIKE ' . $this->_db->Quote('%' . $this->_db->getEscaped($search, true).'%', false);
			}
			if ($filter_state) {
				if ($filter_state == 'P') {
					$where[] = '`published` = 1';
    ...
    public function getRelatedProducts ($id) {
        $query = 'SELECT a.product_id, a.product_name FROM #__cp_transfers_param2 b JOIN #__cp_transfers_info a ' . 
            'ON b.product_id = a.product_id WHERE b.param2_id = ' . $id . ' ORDER BY 2 ASC';
