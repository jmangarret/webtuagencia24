 ...
 *
 * @version $Id: hotels.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelHotels extends CatalogoPlanesModelBasic {

    function __construct() {
        $option = JRequest::getCmd('option');
        $this->_filter_prefix = $option . '_hotels';
        parent::__construct();
    }
	...
	protected function _buildQuery($use_filter = true, $conditions = array()) {
		...
		$query = ' SELECT a.*, m.name AS editor, b.country_name, c.region_name,';
		$query .= ' d.city_name, g.title AS groupname FROM ' . $this->getTable()->getTableName();
                $query .= ' a JOIN #__cp_prm_hotels_category e ON a.category_id = e.category_id ';
                ...
                $query .= ' LEFT JOIN #__cp_hotels_tourismtype f ON a.product_id = f.product_id ';
	...
	public function getOrderByField($query = true) {
		...
		$allowed_fields = array('product_id', 'published', 'product_name', 'ordering', 'stars', 'featured', 'access', 'modified', 'city_name', 'editor');
    ...
    public function store() {
        ...
        $data['zone_id'] = $data['detailszone_id'];
        ...
        $data['additional_description'] = $details['additional_description'];
        ...
        $data['stars'] = ($details['stars'])? $details['stars']: 0;
        ...
        // Cada suplemento se aplica una sola vez por defecto
        if (!empty($data['supplements'])) {
        	foreach ($data['supplements'] as $key => $rowSupplement) {
        		if (!isset($rowSupplement['apply_once'])) $data['supplements'][$key]['apply_once'] = 1;
        	}
        }
        ...
        if (!$row->store()) {
	        ...
	        if ($product_id && isset($data['mediafiles']) && is_array($data['mediafiles'])) {
	            ...
	            $query = 'DELETE FROM #__cp_hotels_files WHERE `file_url` NOT IN (' . $files . ') AND `product_id` = ' . $product_id;
	            ...
	            foreach ($mediafiles as $file) {
	                ...
	                $query = 'SELECT `file_url` FROM #__cp_hotels_files WHERE `file_url` = \'' . $file . '\' AND `product_id` = ' . $product_id;
	                $this->_db->setQuery($query);
	                $this->_db->query();
	                if (!$this->_db->getNumRows()) {
	                    $query = 'INSERT INTO #__cp_hotels_files (`product_id`, `file_url`, `ordering`) VALUES (' . $product_id . ', \'' . $file . '\', ' . $i . ')';
	                } else {
	                    $query = 'UPDATE #__cp_hotels_files SET `ordering` = ' . $i . ' WHERE `file_url` = \'' . $file . '\' AND `product_id` = ' . $product_id;
	                }
	                $this->_db->setQuery($query);
	                if (!$this->_db->query()) {
	                    $this->setError($this->_db->getErrorMsg());
	            ...
	            $query = 'DELETE FROM #__cp_hotels_files WHERE product_id = ' . $product_id;
	            $this->_db->setQuery($query);
	            if (!$this->_db->query()) {
	                $this->setError($this->_db->getErrorMsg());
	                return false;
    ...
    public function getRelatedSupplements($product_id, $getTaxes = true) {
    	if ($getTaxes) {
	        $query = 'SELECT a.*, b.apply_once, d.tax_id, d.tax_name, d.tax_value, d.published as tax_published 
		        FROM #__cp_prm_supplement a INNER JOIN #__cp_hotels_supplement b ON a.supplement_id 
		        = b.supplement_id LEFT JOIN #__cp_hotels_supplement_tax c ON a.supplement_id = c.supplement_id AND 
            ...
            $query = 'SELECT a.* FROM #__cp_prm_supplement a INNER JOIN #__cp_hotels_supplement b 
    ...
    public function getRateParams($product_id) {
    	...
    	// Parámetro 1
        $query = 'SELECT a.* FROM #__cp_prm_hotels_param1 a INNER JOIN #__cp_hotels_param1 b 
    	...
    	// Parámetro 2
        $query = 'SELECT a.* FROM #__cp_prm_hotels_param2 a INNER JOIN #__cp_hotels_param2 b 
    	...
    	// Parámetro 3
        $query = 'SELECT a.* FROM #__cp_prm_hotels_param3 a INNER JOIN #__cp_hotels_param3 b 
    ...
    public function getStockParams($product_id) {
    	// Parámetro 1
        $query = 'SELECT a.param1_id AS param_id, a.param1_name AS param_name FROM #__cp_prm_hotels_param1 a INNER JOIN 
	        #__cp_hotels_param1 b ON a.param1_id = b.param1_id WHERE b.product_id = ' . $product_id . ' ORDER BY a.param1_name ASC';
        $this->_db->setQuery($query);
    ...
    public function getStock($product_id, $start_date, $end_date) {
    	$list = array();
        $query = 'SELECT param_id, `day`, quantity FROM #__cp_hotels_stock WHERE product_id = ' . $product_id .
    ...
    function saveAutomaticStock($product_id, $params, $start_date, $end_date, $quantity) {
    	...
    	foreach ($params as $param_id) {
            ...
            $query .= "CALL CleanHotelStock($product_id, $param_id, '$start_date', '$end_date');\n";
    		// Sólo se almacenan valores mayor o igual a 1 y fechas no vencidas
    		if ($quantity < 1) continue;
    		$query .= "CALL LoadHotelStock($product_id, $param_id, '$start_date', '$end_date', $quantity);\n";
    	}
    ...
    function saveManualStock($product_id, $stock, $start_date, $end_date) {
        ...
        $query .= "CALL CleanHotelStock($product_id, 0, '$start_date', '$end_date');\n";
    	...
    	foreach ($stock as $param_id => $values) {
    		foreach ($values as $date => $quantity) {
                ...
                $query .= "CALL LoadHotelStock($product_id, $param_id, '$date', '$date', $quantity);\n";
    		}
    	}
    	// Se finaliza la transacción
    	$query .= "COMMIT;";
