...
<form action="index.php" method="post" name="adminForm">
	<div id="editcell">
		<fieldset class="adminform">
			<legend><?php echo JText::_('LNG_RESERVATIONS_INCOME_REPORT',true); ?></legend>
			<div style='text-align:left'>
				...
				<select name='hotel_id' id='hotel_id' style='width:300px'
					...
					<option value=0 <?php if ($this->hotel_id ==0)  echo 'selected'?>><?php echo JText::_('LNG_SELECT_DEFAULT',true)?></option>
					<?php
	...
	<input type="hidden" name="task" value="reservationsreports.incomeReport" />
	<input type="hidden" name="refreshScreen" id="refreshScreen" value="<?php echo JRequest::getVar('refreshScreen',null)?>" />
	<?php echo JHTML::_( 'form.token' ); ?> 
</form>


<div id="chartdiv" style="height:400px;width:1200px;margin:0 auto;">
</div>


<script language="javascript" type="text/javascript">
	<?php if(count($this->hotels)==1){?>
	<?php }?>
...
function generateChart(){
	...
	var currencyCode = '<?php echo isset($this->hotel->currency_symbol)?$this->hotel->currency_symbol:"";?>';
	...
	var jsonurl = siteRoot+'administrator/index.php?option='+compName+'&task=reservationsreports.getJsonIncomeData'+inputParams;

	 var labels = ["Income","Second Income"];
	...
	jQuery.ajax({
		...
		success: function(data) {
			...
			if(data==""){
				jQuery('#chartdiv').val('No data  found')
			}
		},
	...
	var plot2 = jQuery.jqplot('chartdiv', jsonurl,{
    	title: '<?php echo JText::_('LNG_HOTEL_INCOME_REPORT',true)?>',
    	...
    	axes: {
	           ...
	           xaxis:{
		          ...
		          tickInterval:'1 '+jQuery('#filter_report_type').val().toLowerCase(),
