 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerZone extends JController {
	...
	function edit() {
		...
		if ($id && empty($data->zone_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=zone', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=zone';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=zone', $msg, $type);
	}
