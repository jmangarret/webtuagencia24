...
/**
 * planparam3 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerPlanParam3 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('planparam3');
		$view = & $this->getView('planparam3', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param3_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=planparam3', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=planparam3';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=planparam3', $msg, $type);
	}
