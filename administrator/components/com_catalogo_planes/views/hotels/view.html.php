...
class CatalogoPlanesViewHotels extends JView {
	...
	$option = JRequest::getCmd('option');
        ...
        switch ($layout) {
            case 'form':
                ...
                // Agregar script para estrellas de alojamiento.
                JHTML::script('jquery.rating.pack.js', COM_CATALOGO_PLANES_BASEURL . 'assets/js/');
                JHTML::stylesheet('jquery.rating.css', COM_CATALOGO_PLANES_BASEURL . 'assets/css/');
                ...
                $title .= ' ' . JText::_('CP.HOTEL');
                // Se colocan los botones en la barra de herramientas
                if (!$isNew) {
                    $bar = & JToolBar::getInstance('toolbar');
                    $bar->appendButton('Link', 'stock', JText::_('CP.STOCK'), 'index.php?option=' . $option . '&view=hotels&task=showstock&cid[]=' . $data->product_id);
                    $bar->appendButton('Link', 'rates', JText::_('CP.RATES'), 'index.php?option=' . $option . '&view=hotelrate&task=listrates&product_id=' . $data->product_id);
                ...
                $form = &JForm::getInstance('',JPATH_COMPONENT.DS.'models'.DS.'hotels.xml');
                //echo JPATH_COMPONENT.DS.'models'.DS.'hotels.xml';
                //print_r($form);die();
                ...
                $form->setValue('stars','details', $data->stars);
            ...
            case 'default_stock':
                ...
                // Obtiene título de la interfaz
                $title = JText::_('CP.STOCK') . ' :: <a href="index.php?option=' . $option . '&view=hotels&task=edit&cid[]=' . $data->product_id . '" class="product_link">' . $this->product_name . '</a>';
                $icon = 'stock.png';
		...
		break;
            ...
            default:
                ...
                //echo 'JSubMenuHelper='.print_r(JToolBarHelper);die();
                // Obtiene título de la interfaz
                $title = JText::_('CP.CATALOGOPLANES') . ' :: ' . JText::_('CP.HOTELS');
                $icon = 'hotel.png';
                break;
        }
