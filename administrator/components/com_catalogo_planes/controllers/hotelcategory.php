 ...
 *
 * @version $Id: hotelcategory.php 2012-09-11 8:29:50 svn $
 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotelCategory extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('hotelcategory');
		$view = & $this->getView('hotelcategory', 'html');
	...
	function edit() {
		...
		if ($cid[0] && empty($data->category_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotelcategory', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotelcategory';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=hotelcategory', $msg, $type);
	}
