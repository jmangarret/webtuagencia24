<?php
/**
 * Joomla! 1.5 component catalogo_cares
 *
 * @version $Id: view.html.php 2012-09-10 18:29:50 svn $
 * @author Yusely Palacios
 * @package Joomla
 * @subpackage catalogo_cares
...
class CatalogoPlanesViewCars extends JView {
...
function display ($tpl = null) {
    ...
    switch ($layout) {
        case 'form':
            ...
            $title .= ' ' . JText::_('CP.CAR');
                    // Se colocan los botones en la barra de herramientas
            if (!$isNew) {
                $bar = & JToolBar::getInstance('toolbar');
                $bar->appendButton('Link', 'stock', JText::_('CP.STOCK'), 'index.php?option=' . $option . '&view=cars&task=showstock&cid[]=' . $data->product_id);
                $bar->appendButton('Link', 'rates', JText::_('CP.RATES'), 'index.php?option=' . $option . '&view=carrate&task=listrates&product_id=' . $data->product_id);
            ...
            $form = &JForm::getInstance('',JPATH_COMPONENT.DS.'models'.DS.'cars.xml');
        ...
        case 'default_stock':
            $data = $this->get('data');
            // Obtiene título de la interfaz
            $title = JText::_('CP.STOCK') . ' :: <a href="index.php?option=' . $option . '&view=cars&task=edit&cid[]=' . $data->product_id . '" class="product_link">' . $this->product_name . '</a>';
            $icon = 'stock.png';
        ...
        default:
            ...
            // Obtiene título de la interfaz
            $title = JText::_('CP.CATALOGOPLANES') . ' :: ' . JText::_('CP.CARS');
            $icon = 'car.png';
            break;
        }
