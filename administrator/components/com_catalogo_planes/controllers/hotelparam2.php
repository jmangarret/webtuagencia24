...
/**
 * hotelparam2 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotelParam2 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('hotelparam2');
		$view = & $this->getView('hotelparam2', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param2_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotelparam2', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotelparam2';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=hotelparam2', $msg, $type);
	}
