 ...
 *
 * @version $Id: tourrate.php 2012-09-11 8:29:50 svn $
...
* @subpackage		catalogo_planes
*/
class TableTourRate extends BasicTable {
	...
	function __construct(& $db) {
        ...
        $this->relations['prices']['referencedtable'] = '#__cp_tours_rate_price';
        $this->relations['prices']['referencedfield'] = 'rate_id';
        $this->relations['prices']['extrainfofields'] = array('param1', 'param2', 'price');

        $this->relations['supplements'] = array();
        $this->relations['supplements']['referencedtable'] = '#__cp_tours_rate_supplement';
        $this->relations['supplements']['referencedfield'] = 'rate_id';
        $this->relations['supplements']['extrainfofields'] = array('amount');

        parent::__construct('#__cp_tours_rate', 'rate_id', $db);
	}
    ...
    public function load($id = null, $loadRelationships = true) {
        // Tourgar la información de la tarifa junto con la temporada
        ...
        $query .= ' a JOIN #__cp_tours_info p ON a.product_id = p.product_id JOIN';
        ...
        if ($result = $this->_db->loadAssoc()) {
	        ...
	        // Si se tourgó correctamente y hay un id y relaciones definidas, tourgue los datos de las relaciones.
    ...
    function store($updateNulls = false, $updateRelationships = true) {
        ...
        if ($result && $this->product_id) {
	        ...
	        // se toma de la configuración del tarifario. Colotour en 1 para usar las vigencias.
	        $calculatePrices = 0;
            $query = 'CALL GenerateToursRateResumeById(' . $this->product_id . ', CURDATE(), ADDDATE(CURDATE(), 365), ' . $calculatePrices . ')';
            $this->_db->setQuery($query);
            if (!$this->_db->query()) {
            	$this->setError($this->_db->getErrorMsg());
            	$result = false;
