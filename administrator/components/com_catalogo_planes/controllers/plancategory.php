 ...
 *
 * @version $Id: plancategory.php 2012-09-11 8:29:50 svn $
...
/**
 * plancategory Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerPlanCategory extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('plancategory');
		$view = & $this->getView('plancategory', 'html');
	...
	function edit() {
		...
		if ($cid[0] && empty($data->category_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=plancategory', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=plancategory';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=plancategory', $msg, $type);
	}
