...
class JHotelUtil{
	...
	public static function getDateGeneralFormat($data){
		...
		/*CODIGO CREADO PARA CAMBIAR FECHA AL ESPAÑOL.
		AUTOR: YANUEL LEAL.
		*/
...
for($i=0;$i<strlen($dateS);$i++){ //Ciclo substraer día

	if ($dateS{$i} == ' '){

		for($ii=$i+1;$ii<strlen($dateS);$ii++){//Ciclo substraer fecha

			if ($dateS{$ii} == ' '){

				for($iii=$ii+1;$iii<strlen($dateS);$iii++){//Ciclo Substraer mes

					if ($dateS{$iii} == ' '){

						for($iiii=$iii+1;$iiii<strlen($dateS);$iiii++){//Ciclo substraer año

								$año.=$dateS{$iiii};
								$año = substr($año, -4); //Caso cuando la fecha tiene un solo dígito, problema con años
								$iii=strlen($dateS);
							
						}

					}else{
						if ($dateS{$iii}=="1" || $dateS{$iii}=="2" || $dateS{$iii}=="3" || $dateS{$iii}=="4"
							 || $dateS{$iii}=="5" || $dateS{$iii}=="6" || $dateS{$iii}=="7"
							 || $dateS{$iii}=="8" || $dateS{$iii}=="9"){
							$iii+1; //Necesario cuando la fecha tiene un solo dígito, problema con meses
						}else{
						$mes.=$dateS{$iii};
						$ii=strlen($dateS);
							}
...
foreach ($dayNames as $keyd => $value) {
	if ($dia == $value){
		$diaE = $keyd;
	}
}
...
foreach ($monthNames as $keym => $value) {
	if ($mes == $value){
		$mesE = $keym;
	}
}

$fecha = $dias[date('w')*0+$diaE]." ".$fecha2." de ".$meses[date('n')*0+$mesE]. " del ".$año ;
