...
// 		dmp($query);						
	...
	function getHotelSearchQuery($searchParam, $searchType=0){
		...
		$activeHotelsFilter="";
		$startDate= !empty($searchParam['startDate'])?$searchParam['startDate']:null ;
		$endDate = !empty($searchParam['endDate'])?$searchParam['endDate']:null ; 
		$dayFilter = '';
		...
		$query = "
					select 
					h.hotel_id, h.hotel_name, h.hotel_short_description as hotel_description, h.hotel_county, h.hotel_city, h.hotel_address, h.hotel_zipcode, h.hotel_website, h.hotel_stars, h.hotel_rating_score, h.featured, h.hotel_selling_points, h.recommended, h.hotel_phone, h.hotel_number,
					hotelId,
