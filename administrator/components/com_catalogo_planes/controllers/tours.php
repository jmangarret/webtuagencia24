 ...
 *
 * @version $Id: tours.php 2012-09-11 8:29:50 svn $
 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerTours extends JController {
	...
	function display() {
		...
		if ($country_id > 0) {
			...
			$regions = $countryModel->getRegionsList($regionConditions,$productType);//$regionModel->getList(true, $regionConditions);
		...
		$opt = JHTML::_('select.option', '', ' - ' . JText::_('CP.TOURCATEGORY') . ' - ', 'category_id', 'category_name');
		// Se obtiene el modelo de la región
		$categoryModel =& $this->getModel('tourcategory');
		...
		// Se obtiene el modelo para los tipos de turismo
		$tourismtypeModel =& $this->getModel('tourismtype');
		$opt = JHTML::_('select.option', '', ' - ' . JText::_('CP.TOURTOURISMTYPE') . ' - ', 'tourismtype_id', 'tourismtype_name');
	...
	function edit() {
		...
		if ($id && empty($data->product_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=tours', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		...
		if ($data->country_id > 0) {
			...
			if ($data->region_id > 0 && $params->cfg_use_regions == '1') {
		...
		$taxes = $taxesModel->getList(true, array('published' => 1, 'product_type_code' => 'tours'));
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=tours';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=tours', $msg, $type);
	}
	...
	function orderdown() {
		...
		$model =& $this->getModel('tours');
		$model->move($cid, 1);
		$this->setRedirect('index.php?option=' . $option . '&view=tours');
	}
	...
	function access() {
		...
		// Lanzar error si no se ha seleccionado al menos un registro a modifitour
		if (empty($cids)) {
			JError::raiseError(500, JText::_('CP.SELECT_AN_ITEM_TO_EDIT'));
		}
	...
	function duplicate() {
		...
		// Lanzar error si no se ha seleccionado al menos un registro a duplitour
		if ($cid < 1) {
		...
		// Si se logró duplitour el producto, redireccionar a la página de edición
		if ($product_id > 0) {
			$msg = JText::_('CP.DATA_SAVED');
			$type = 'message';
		} else {
	...
	function listrates() {
		...
		// Redireccionar
		$this->setRedirect('index.php?option=' . $option . '&view=tourrate&product_id=' . $cid . '&season_year=' . date('Y'));
	}
	...
	function autoloadstock() {
		...
		// Lanzar error si no se ha seleccionado un registro a modifitour su stock
		if ($product_id < 1) {
			JError::raiseError(500, JText::_('CP.SELECT_AN_ITEM_TO_UPDATE_STOCK'));
		}

		// Verifitour que se tengan los datos adecuados
		$params = JRequest::getVar('param_id', array(0), 'method', 'array');
		if (count($params) < 1) {
			JError::raiseError(500, JText::_('CP.TOUR_ERROR_EMPTY_PARAM1'));
		}
