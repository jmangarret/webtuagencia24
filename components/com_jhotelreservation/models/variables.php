...
class JHotelReservationModelVariables extends JModelLegacy
{
	...
	public function checkRoomAvailability(&$rooms,$items_reserved, $hotel_id, $datas ,$datae){
		...
		//dump("T");
		//dump($temporaryReservedRooms);
		
		foreach($rooms as $room){
		//	dump("NR: ".$room->room_id." ".$room->nrRoomsAvailable);
			//dump($room->daily);
			...
			foreach( $room_available_ids as $k_room => $val_room )
			{
				...
				foreach( $rooms as $key => $value )
				{
					...
					// if( count($room_nr_availables_tmp) <= $value->room_number_reserved)
						// continue;
					//dbg( $value->numbers_available.' >> '.$value->room_number_reserved);
...
// 						dump($value->offer_name);
// 						dump($value->offer_datas);
// 						dump($value->offer_datae);
// 						dump("start");
...
// 						dump($rooms[$key]->is_disabled==true?"true":"false");
					}
					//~check if we have continue time period	
					$no_continuitate 		= false;
					$day_price 				= 0;
					...
					for( $d = strtotime($datas);$d < strtotime($datae); )
					{
						...
						else
						{
							$query = "  SELECT 
											...
											code
										FROM #__hotelreservation_rooms_discounts 
										WHERE 
											is_available = 1 
											AND
...
// 									$v->offer_room_discount_type	= $v->offer_price_type_extranights;
					...
					if( $check_room_number )
					{
						...
						//dump($value->offer_name);
						//dump($resN);
						//dump($rooms_numbers_already_selected);
						 if(count($resN) == 0 ){
							$rooms[$key]->is_disabled = true;
						 }
						 else
						foreach( $resN AS $valN )
						{
							...
							foreach($arr_dates as $kDate => $str_data )
							{
								...
								// dbg($val);
								// dbg($disc);
								$price  = my_round($val - my_round($val * ($disc/100),2),2);
								// dbg($price);
					...
					if( count( $rooms[$key]->daily_medium_prices ) > 0 )
					{
						...
						$rooms[$key]->room_average_price = fmt(my_round($total/count($rooms[$key]->daily_medium_prices),2),2);
					}
					else
					{
						$rooms[$key]->room_average_price = fmt($day_price,2);
						//dbg($day_price);
					}
	...
	function &getPackages( $package_ids )
	{
		...
		$packages = array();
		// dbg($this->itemPackageNumbers);
		// exit;
		
		foreach( $package_ids as  $pck )
		{
			// dbg($pck);
			// dbg($arr_selected_packages_days);
			...
			foreach( $pack_res as $key => $value )
			{
				...
				else
				{
					if( $value->is_price_day==true )
						$packages[$key]->price_final = my_round($price_final/count($packages[$key]->daily),2);
				}
			}
	...
	function getReservationDetails($modelData, $showDisplayPrice = false)
	{
		  ...
		  <thead>
			...
			<tbody bgcolor="#F8F7F5" class='rsv_dtls_container'>
				...
				<tr>
					...
					<td style="padding: 3px 9px;" colspan="4" align="left">
						<span class='title_ID'><?php echo $modelData->getStringIDConfirmation()?></span>
					</td>
				</tr>
				<?php
				}
				...
				// $val_offers_rooms 		= 0;
				$counter = 0;
				// dbg(count($modelData->items_reserved));
				// dbg($this->itemRoomsSelected);
				// exit;
				...
				{				
					...
					foreach( $modelData->itemRoomsSelected as $room )
					{
						...
						foreach( $room->daily as $day)
						{
							...
							foreach( $day['discounts'] as $d )
							{
								if($d->maximum_number_days >=$dayCounter){
									...
									if( strlen($info_discount)>0)
										$info_discount	.='<BR>';
									$info_discount	.= $d->discount_name.' '.fmt(-1 * $d->discount_value).''.($d->percent==1?"%":" ".$modelData->itemCurrency->description);
								}
							}
							...
							<tr class='rsv_dtls_room_info'>
								...
								<td align="left" valign="top" style="border-top:solid <?php echo $room->offer_id==0? " 2px black" : " 1px grey"?>;padding: 3px 9px;" nowrap="nowrap">
									...
									<?php echo getDateGeneralFormat($day['data']);?>
									<?php
								
										//echo strlen($info_discount)>0 ? (fmt($price_day,2)."<strong> x </strong>".$modelData->itemRoomsCapacity[ $room->room_id ][1]."".($modelData->itemRoomsCapacity[ $room->room_id ][1] >1? JText::_('LNG_ROOMS',true):  JText::_('LNG_ROOM',true))):'';
										echo $info_discount;
								...
								<td align="right" valign="top" style="border-top:solid <?php echo $room->offer_id==0? " 2px black" : " 1px grey"?>;padding: 3px 9px;">
									&nbsp;
									<?php
									echo fmt($price_day * /*$days_dif * */$modelData->itemRoomsCapacity[ $room->room_id ][1],2);
									?>
								</td>
						...
						foreach( $itemOptions as $option )
						{
							...
							<tr>
								...
								<td align="right" nowrap style="padding: 3px 9px">
									...
									{
										echo fmt($showDisplayPrice == true? $option->option_display_price: $option->option_price);
									}
									?>
								</td>
							</tr>
						...
						if( ($room->offer_id == 0 || $nr_days_except_offers > 0 ) && $val_room + $val_options != 0 )
						{
							...
							<tr class='rsv_dtls_subtotal'  bgcolor="#EFEDE9">
								...
								<td align=right style="padding: 3px 9px;">
									<strong><?php echo fmt($val_room + $val_options,2)?></strong>
								</td>
						...
						foreach( $modelData->itemPackages as $package)
						{
							...
							// dbg($package->offer_id 		.'!= '.$room->offer_id );
							// dbg($package->room_id 		.'!= '.$room->room_id );
							// dbg($package->current 		.'!= '.$room->current );
								...
								?>
									<tr>
										...
										<td align=right style="padding: 3px 9px;">
											...
											if( $day['is_offer']==false )
											{
												echo fmt(($showDisplayPrice == true? $day['display_price_final']: $day['price_final']) * $modelData->itemPackageNumbers[$keyPackage][4]);
											}
											?>
										</td>
							...
							else
							{
								...
								<tr>
									...
									<td align=right style="padding: 3px 9px;">
										...
										if( $package->offer_id ==0 )
										{
											echo fmt(($showDisplayPrice == true? $package->display_price_final: $package->price_final) * $modelData->itemPackageNumbers[$keyPackage][4]);
										}
										?>
									</td>
								</tr>
						...
						if( $val_packages != 0 )
						{
							...
							<tr class='rsv_dtls_package_price'  bgcolor="#EFEDE9">
								...
								<td align=right style="padding: 3px 9px;">
									<strong><?php echo fmt($val_packages,2)?></strong>
								</td>
						...
						// dbg($modelData->itemPackages);
						// exit;
						//dbg($modelData->extraOptions);
						...
						if( isset($modelData->extraOptions) && count($modelData->extraOptions) > 0 && count($extraOptionsIds)>0 ){ 
							...
							foreach( $modelData->extraOptions as $extraOption){
								...
								<tr>
									<td nowrap align=left colspan=6 style="padding: 3px 9px 3px 20px;">
										<?php
											echo $extraOption->name.", ".$this->itemCurrency->currency_symbol." ". fmt($extraOption->price,2)." ". ($extraOption->price_type == 1?strtolower(JText::_('LNG_PER_PERSON',true))." ":"" )."".($extraOption->is_per_day == 1 ?strtolower(JText::_('LNG_PER_DAY',true)):"" )."".($extraOption->is_per_day == 2 ?strtolower(JText::_('LNG_PER_NIGHT',true)):"" );
							...
							<tr class='rsv_dtls_room_price' bgcolor="#EFEDE9">
								...
								<td align="right" style="padding: 3px 9px" >
									<strong><?php echo fmt($extraOptionsAmount,2)?></strong>
								</td>
						...
						if( count( $modelData->airport_transfer_type_ids ) > 0 )
						{
							...
							foreach( $modelData->itemAirportTransferTypes as $keyAirportTransfer => $airport_transfer )
							{
								...
								<tr >	
									...
									<td align=right style="padding: 3px 9px;" >
										<?php echo fmt($val_airport_transfer)?>
									</td>
								...
								<tr class='rsv_dtls_room_price' bgcolor="#EFEDE9">
									...
									<td align="right" style="padding: 3px 9px" >
										<strong><?php echo fmt($val_airport_transfer,2)?></strong>
									</td>
						...
						<tr class='rsv_dtls_subtotal'  bgcolor="#EFEDE9" style="display:none">
							...
							<td align=right style="padding: 3px 9px;">
								<strong><?php echo fmt($val_room,2)?></strong>
							</td>
				...
				<tr class='rsv_dtls_total_room_price' bgcolor="#EFEDE9">
					...
					<td align="right" style="border-top:solid 2px black;padding: 3px 9px" >
						<strong><?php echo fmt($val_rooms)?></strong>
					</td>
				...
				if( $bIsCostV )
				{
					...
					<tr class='rsv_dtls_subtotal' bgcolor="#EFEDE9">
						...
						<td align=right style="padding: 3px 9px;"><strong><?php echo fmt( $costV,2)?>
						</strong>
						</td>
					</tr>
					<?php
				 ...
				 if( $bIsCostV )
				 {
					...
					<tr class='rsv_dtls_subtotal' bgcolor="#EFEDE9">
						...
						<td align=right style="padding: 3px 9px;"><strong><?php echo fmt( $val_rooms,2)?>
						</strong>
						</td>
					</tr>
					<?php
			...
			foreach($arr_val_amount_to_pay  as $v)
			{
				...
				<tr class='rsv_dtls_subtotal'  bgcolor="#EFEDE9">
					...
					<td align=right style="padding: 3px 9px;">
						<strong><?php echo fmt( $v['val'] + $v['proc']['p'] * $v['room'] /100,2)?></strong>
					</td>
				</tr>
				<?php
				}
			...
			if( count($arr_val_amount_to_pay) > 1)
			{
				...
				<tr class='rsv_dtls_subtotal'  bgcolor="#EFEDE9">
					...
					<td align=right style="padding: 3px 9px;">
						<strong><?php echo fmt( $val_costs,2)?></strong>
					</td>
				</tr>
				<?php
			}
			...
			if( $val_costs > 0  && $this->tip_oper < 5 )
			{
				...
				<tr class='rsv_dtls_subtotal'  bgcolor="#EFEDE9">
					...
					<td align=right style="padding: 3px 9px;">
						<strong><?php echo fmt( $val_rooms - $val_costs,2)?></strong>
					</td>
				</tr>
				<?php
			}
			...
			if( count($modelData->itemPayments) > 0 )
			{
				...
				foreach( $modelData->itemPayments as $pay)
				{
					...
					if( $pay->payment_type_id == DONE_PAYMENT_ID &&  $pay->tip =='')
					{
						...
						$pay->payment_value		= my_round($total  - $total_pay_ok,2);
						$sign = "-";
						...
						else
						{
							...
							else
							{
								...
								if( $pay->payment_percent !=0 )
									$val_pay +=  my_round($total * $pay->payment_percent / 100,2) ;
					...
					if( $pay->payment_type_id != CANCELED_PAYMENT_ID  )
					{
						...
						switch( $pay->payment_status )
						{
							...
							case PAYMENT_STATUS_WAITING:
								...
								{
									$val_pay_ok +=  my_round($total * $pay->payment_percent / 100,2) ;
								}
						...
						{
							$str_val_temporary	+= my_round($total * $pay->payment_percent / 100,2) ;
							if( $pay->payment_status != PAYMENT_STATUS_PAYED )	
							{
								$str_val_temporary  = "<i>".fmt(my_round($total * $pay->payment_percent / 100,2))."</i>";
								$total_pay_pending += my_round($total * $pay->payment_percent / 100,2) ;
							}
						}
						...
						)
						{
							...
							if( $pay->payment_status != PAYMENT_STATUS_PAYED )	
							{
								$str_val_temporary 	= "<i>".fmt( $pay->payment_value)."</i>";
				...
				<tr class="confirmation_card_information" bgcolor="#dee5e8">
					...
					<td align=right style="padding: 3px 9px">
						<strong>
							<?php echo $val_pay_ok != 0 ? fmt($val_pay_ok) : (strlen($str_val_temporary) > 0 ? $str_val_temporary: "&nbsp;" ) ?>
						</strong>
					</td>
				</tr>
				<?php
				...
				$remaining_payment =  my_round($val_rooms+$total_payments-$total_payments_ok,2)	;	   
				
				$remaining_payment -= $total_pay_pending;
				//dbg($total_pay_pending);
				...
				<tr bgcolor="#dee5e8" class="confirmation_total" >
					...
					<td align=right style="padding: 3px 9px">
						<strong><?php echo $remaining_payment!= 0.00 ? fmt($remaining_payment, 2) : "&nbsp;"?></strong>
					</td>
				</tr>
		...
		$modelData->payment_variables->amount 					= my_round( 
	...
	{
		//dbg($this->itemPaymentProcessors);
		// exit;
		ob_start();
		...
		// dbg($this->payment_processor_sel_type);
		// dbg($arr_payment_type);
		// dbg($arr_conf_payment);
		// exit;
			...
			switch($type)
			{
				case PROCESSOR_PAYFLOW:
					...
					if( isset($arr_conf_payment[$id]) )
					{
						...
						{
							echo my_round($arr_conf_payment[$id]->payment_value,2).' &nbsp;';
						}
					}
					break;
				...
				case PROCESSOR_BANK_ORDER:
					...
					$str = str_replace(EMAIL_RESERVATION_COST, 					    fmt( ($this->total_cost > 0 ? $this->total_cost : $this->total - $this->total_payed),2),	$str);
					echo $str;
...
//				$return = JMail::sendMail($this->itemHotelSelected->email,$this->itemHotelSelected->hotel_name, $data['email'], $emailSubject, $emailBody);
			...
			if( getCurrentJoomlaVersion() > 1.5 )
			{
...
// 		dbg($this);
	...
	function getStringRoomsDiscounts($item)
	{
		...
		foreach($item as $key => $value )
		{
			if( $str !='')
				$str .= ',';
			$str .=$value->room_discount_id."|".$value->room_id.'|'.$value->days;
	...
	*/
		...
		if( $is_offer )
		{
			...
			foreach( $daily as $key => $day )
			{
				if( count($day['numbers']) == 0 )
				{
					...
					//dbg($disc);
					//dbg($val);
					$pret = my_round($val - my_round($val * ($disc/100),2),2);
				}
				else
				{
					$val = 0;
					...
					$pret = my_round($val/count($day['numbers']),2);
				}
				// echo "<HR>";
				// dbg($day);
		...
		else 
		{
			foreach( $daily as $key => $day )
			{
				if( count($day['numbers']) == 0 )
				{
					...
					// $pret = my_round($val*100/(100+$disc),2);
					if( $disc != 0 )
						$pret = my_round($val - my_round($val * ($disc/100),2),2);
					else
						$pret = my_round($val,2);
	...
	function displayPaymentProcessors( $show_cash_payment = false, $ignoredProcessorTypes = array() )
	{
		...
		//dump($this->itemPaymentProcessors);
				...
				<TD>
					...
					>
						...
						else if( $proc->paymentprocessor_type == PROCESSOR_BANK_ORDER )
						{
							...
							$str = str_replace(EMAIL_RESERVATION_COST, 					fmt( $this->total_cost > 0 ? $this->total_cost : $this->total - $this->total_payed,2),	$str);
							//$str = str_replace(EMAIL_RESERVATION_ID, 					$this->getStringIDConfirmation(),	$str);
							echo $str;
	...
	function getStringIDConfirmation()
	{
	...
	function prepareVariablesPayPalExpress( )
	{
		...
		if( $row->paymentprocessor_type == PROCESSOR_PAYPAL_EXPRESS )
		{
			...
			?>
				...
				<input type='hidden' name='item_number'		id= 'item_number' 	value='<?php echo $this->getStringIDConfirmation()?>'/>
				...
				<input type='hidden' name='amount' 			id= 'amount' 		value='<?php echo my_round( $this->total_cost > 0 ? $this->total_cost : $this->total - $this->total_payed,2)?>'/>
	...
	static function writeMessage($message, $tip=0)
	{
			...
			if( $array_conditions['operation'] == 'edit' )
			{
				...
				if( is_array($array_conditions['skip_value']) && count($array_conditions['skip_value']) > 0)
				{
					...
					else
					{
						...
						// dbg($v_cmp);
						// dbg($array_conditions['skip_value']);
						for( $i = 0; $i < count( $array_conditions['skip_value'] ); $i++ )
						{
							if( isset( $v_cmp[ $i ] ) && $v_cmp[ $i ] != $array_conditions['skip_value'][$i] )
							{
