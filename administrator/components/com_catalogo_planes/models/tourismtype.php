 ...
 *
 * @version $Id: tourismtype.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelTourismType extends CatalogoPlanesModelBasic {
	...
	protected function _buildQuery($use_filter = true, $conditions = array()) {
		// Obtiene el WHERE y ORDER BY para la consulta
                ...
                $orderby    = ($use_filter && (count($conditions) < 1))? $this->_buildQueryOrderBy(): ' ORDER BY tourismtype_name ASC';
	...
	public function getOrderByField() {
		...
		$filter_order = $mainframe->getUserStateFromRequest($this->_filter_prefix . 'filter_order', 'filter_order', 'tourismtype_name', 'cmd');

		// se asegura que el orden esté dando por los campos aprobados
		if (!in_array($filter_order, array('tourismtype_id', 'tourismtype_name', 'published'))) {
			$filter_order = 'tourismtype_name';
		}
	...
	protected function _buildQueryWhere($conditions = array()) {
        ...
        if (count($conditions) > 0) {
            ...
            $allowed_filters['tourismtype_name'] = '`tourismtype_name`';
			...
			if ($search) {
				$where[] = ' LOWER(`tourismtype_name`) LIKE ' . $this->_db->Quote('%' . $this->_db->getEscaped($search, true).'%', false);
			}
			if ($filter_state) {
				if ($filter_state == 'P') {
					$where[] = '`published` = 1';
