...
/**
 * planparam2 Controller
 *
 * @package Joomla
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerPlanParam2 extends JController {
	...
	function display() {
		...
		// obtiene el modelo
		$model =& $this->getModel('planparam2');
		$view = & $this->getView('planparam2', 'html');
	...
	function edit() {
		...
		if ($id && empty($data->param2_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=planparam2', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=planparam2';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=planparam2', $msg, $type);
	}
