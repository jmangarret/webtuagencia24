	...
	if(jQuery('.trigger').length > 0) 
	{
		jQuery('.trigger').click(function() 
		{
			if (jQuery(this).hasClass('open')) 
			{
				...
				jQuery(this).children('.link_more').html('&nbsp;<?php echo "Fotos"?> »');
				return false;
			} else {
...
jQuery(window).load(function(){
	<?php if(JRequest::getVar("room_id",null)!=null) {?>
		jQuery("#room_<?php echo JRequest::getVar('room_id')?> .trigger").trigger('click');
		setTimeout(function(){
			jQuery("td#room_<?php echo JRequest::getVar('room_id')?>").focus();
		},  1000);
	<?php } ?>
});
...
<form autocomplete='off' action="<?php echo JRoute::_('index.php?option=com_jhotelreservation') ?>" method="post" name="userForm" id="userForm" >
	<div id="boxes" class="hotel_reservation">
		<div id='div_room'>
			...
			<?php 
				...
				if(count($this->offers)>0 && $this->appSettings->is_enable_offers){
	...
	<script>		
		...
		function showRoomCalendar(hotelId, year, month, identifier){
			...
			<?php 
			/*echo "<pre>";
			var_dump($this->userData->reservedItems);
			echo "</pre>";*/
				foreach($this->userData->reservedItems as $itemReserved){
					echo 'postParameters +="&items_reserved[]='.$itemReserved.'";';
				}
