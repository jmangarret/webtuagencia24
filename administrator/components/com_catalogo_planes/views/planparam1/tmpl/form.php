...
<script type="text/javascript">
    ...
    Joomla.submitbutton = function(pressbutton) {
		...
		if (jQuery.trim(form.param1_name.value) == "") {
			alert("<?php echo JText::_('CP.PLANPARAM1_ERROR_EMPTY_NAME', true); ?>");
		} else {
			submitform(pressbutton);
		}
	}
		...
		<tr>
			<td align="right" class="key">
				<label for="param1_name">
					<?php echo JText::_('CP.PLANPARAM1_NAME'); ?>: *
				</label>
			</td>
			<td>
		...
		<tr>
			<td align="right" class="key">
				<label for="description">
					<?php echo JText::_('CP.PLANPARAM1_DESCRIPTION'); ?>:
				</label>
			</td>
			<td>
				<input class="text_area" type="text" name="description" id="description" size="32" maxlength="255" value="<?php echo $data->description; ?>" />
			</td>
		</tr>
		<tr>
			<td align="right" class="key">
				<label for="value">
					<?php echo JText::_('CP.PLANPARAM1_VALUE'); ?>:
				</label>
			</td>
			<td>
			 <?php
			     ...
			     for ($j = 1; $j < 6; $j++) {
			     	if ($data->value != $j) {
			     	   $content .= '<input type="radio" name="value" id="value" value="' . $j . '" class="star" />';
			     	} else {
			     	   $content .= '<input type="radio" name="value" id="value" value="' . $j . '" checked="checked" class="star" />';
			     	}
			     }
			     echo $content;
			 ?>
