 ...
 *
 * @version $Id: cars.php 2012-09-10 18:29:50 svn $
...
class CatalogoPlanesModelCars extends CatalogoPlanesModelBasic {

    function __construct() {
        $option = JRequest::getCmd('option');

        $this->_filter_prefix = $option . '_cars';
    ...
    protected function _buildQuery($use_filter = true, $conditions = array()) {
        ...
        $query .= ' a JOIN #__cp_prm_cars_category e ON a.category_id = e.category_id ';
    ...
    public function store() {
        ...
        if (!$row->store()) {
            ...
            if ($product_id && isset($data['mediafiles']) && is_array($data['mediafiles'])) {
                ...
                $query = 'DELETE FROM #__cp_cars_files WHERE `file_url` NOT IN (' . $files . ') AND `product_id` = ' . $product_id;
                ...
                foreach ($mediafiles as $file) {
                    ...
                    $query = 'SELECT `file_url` FROM #__cp_cars_files WHERE `file_url` = \'' . $file . '\' AND `product_id` = ' . $product_id;
                    $this->_db->setQuery($query);
                    $this->_db->query();
                    if (!$this->_db->getNumRows()) {
                        $query = 'INSERT INTO #__cp_cars_files (`product_id`, `file_url`, `ordering`) VALUES (' . $product_id . ', \'' . $file . '\', ' . $i . ')';
                    } else {
                        $query = 'UPDATE #__cp_cars_files SET `ordering` = ' . $i . ' WHERE `file_url` = \'' . $file . '\' AND `product_id` = ' . $product_id;
                    }
                    $this->_db->setQuery($query);
                    if (!$this->_db->query()) {
                        $this->setError($this->_db->getErrorMsg());
                ...
                $query = 'DELETE FROM #__cp_cars_files WHERE product_id = ' . $product_id;
                $this->_db->setQuery($query);
                if (!$this->_db->query()) {
                    $this->setError($this->_db->getErrorMsg());
                    return false;
    ...
    public function getRelatedSupplements($product_id, $getTaxes = true) {
        if ($getTaxes) {
            $query = 'SELECT a.*, d.tax_id, d.tax_name, d.tax_value, d.published as tax_published 
                FROM #__cp_prm_supplement a INNER JOIN #__cp_cars_supplement b ON a.supplement_id 
                = b.supplement_id LEFT JOIN #__cp_cars_supplement_tax c ON a.supplement_id = c.supplement_id AND 
            ...
            $query = 'SELECT a.* FROM #__cp_prm_supplement a INNER JOIN #__cp_cars_supplement b 
    ...
    public function getRateParams($product_id) {
        ...
        // Parámetro 1
        $query = 'SELECT a.* FROM #__cp_prm_cars_param1 a INNER JOIN #__cp_cars_param1 b 
        ...
        // Parámetro 2
        $query = 'SELECT a.* FROM #__cp_prm_cars_param2 a INNER JOIN #__cp_cars_param2 b 
    ...
    public function getStockParams($product_id) {
        $list = array();
        $list[] = array('param_id' => 1, 'param_name' => JText::_('CP.CAR'));
    ...
    public function getStock($product_id, $start_date, $end_date) {
        $list = array();
        $query = 'SELECT param_id, `day`, quantity FROM #__cp_cars_stock WHERE product_id = ' . $product_id .
    ...
    function saveAutomaticStock($product_id, $params, $start_date, $end_date, $quantity) {
        ...
        foreach ($params as $param_id) {
            ...
            $query .= "CALL CleanCarStock($product_id, $param_id, '$start_date', '$end_date');\n";
            // Sólo se almacenan valores mayor o igual a 1 y fechas no vencidas
            if ($quantity < 1) continue;
            $query .= "CALL LoadCarStock($product_id, $param_id, '$start_date', '$end_date', $quantity);\n";
        }
    ...
    function saveManualStock($product_id, $stock, $start_date, $end_date) {
        ...
        $query .= "CALL CleanCarStock($product_id, 0, '$start_date', '$end_date');\n";
        ...
        foreach ($stock as $param_id => $values) {
            foreach ($values as $date => $quantity) {
                ...
                $query .= "CALL LoadCarStock($product_id, $param_id, '$date', '$date', $quantity);\n";
            }
        }
        // Se finaliza la transacción
        $query .= "COMMIT;";
