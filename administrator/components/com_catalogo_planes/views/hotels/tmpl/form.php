...
$showRegions = ($this->params->cfg_use_regions_hotel == '1')? true: false;
...
$applyOnceLabel = JText::_('CP.HOTEL_SUPPLEMENT_APPLY_ONCE');
...
<script type="text/javascript">
	            ...
	            var newContent = '<div class="supplement_info"><hr />' +
	                '<table border="0" cellpadding="5">' +
	                    '<tr>' +
	                        ...
	                        '<?php echo $addTaxImg; ?></td></tr></table></div></fieldset></td>' +
	                    '</tr>' +
	                    '<tr>' +
	                        '<td colspan="2"><input type="checkbox" name="supplements[' + id + '][apply_once]" id="supplements[' + id + '][apply_once]" value="0" /><label for="supplements[' + id + '][apply_once]"><?php echo $applyOnceLabel; ?></label></td>' +
	                        '<td align="right"><?php echo $deleteSupplementLabel; ?></td>' +
	                    '</tr>' +
	                '</table>' +
	            '</div>';
	...
	jQuery.noConflict()(document).ready(function ($) { 
        ...
        if ($data->product_id < 1) {
            echo '$(\'#details_disclaimer\').val("' . JText::_('CP.HOTEL_DISCLAIMER_DEFAULT_VALUE') . '");';
        }
        ?>
        ...
        var elementsOrder = ["product_name", "detailscountry_id", "detailsregion_id", "detailscity_id", 
                             "details_zone_id", "details_publish_up", "details_publish_down", 
                             "details_featured", "details_product_desc", "details_additional_description", 
                             "details_product_code", "details_disclaimer", "details_tourismtypes", 
                             "details_access", "detailscategory_id", "detailssupplier_id", 
                             "details_published1", "details_published0", "details_product_url", "rateparam1", 
                             "rateparam2", "rateparam3", "amenities", "selecttaxes"];
      ...
      //Codigo para dejar solo lectura la latitud y longitud
        ...
        $('#details_publish_up').datepicker({
            ...
            beforeShow: function(input, inst) { alert($(this).datepicker('getDate'));
                if (beginsCalendar) { 
            ...
            onSelect: function (selectedDate) { alert('entra');
                var start = $(this).datepicker('getDate');
        ...
        // Agrega la clase necesaria a los radio buttons para que se conviertan en estrellas
        $('input:radio[name="details[stars]"]').each(function () {
            $(this).addClass('star');
        });
        // Convertir los radio buttons en estrellas
        $('input:radio[name="details[stars]"]').rating();
        // Quitar los labels de los radio button que se convertirán en estrellas
        $('#details_stars').find('label').remove();
        ...
        $("#supplement_product_types,#supplement_tourism_types").change(function() {
            ...
            generateSelectByAjax(url, 'supplement_list', 'supplement_id', 'supplement_name', '0', emptySelectOption, assignedSupplementList, "supplement_search_message", "<?php echo JText::_('CP.HOTEL_SUPPLEMENT_SEARCH_NO_RESULTS_MESSAGE', true); ?>");
        });
    ...
    //function submitbutton(pressbutton)  {
        ...
        if (jQuery.trim(form.product_name.value) == "") {
            ...
            alert("<?php echo JText::_('CP.HOTEL_ERROR_EMPTY_HOTELCATEGORY', true); ?>");
            ...
            if (!submit) {
                alert("<?php echo JText::_('CP.HOTEL_ERROR_EMPTY_PARAM1', true); ?>");
                return;
            }
            submit = false;
            ...
            if (!submit) {
                alert("<?php echo JText::_('CP.HOTEL_ERROR_EMPTY_PARAM2', true); ?>");
                return;
            }
            submit = false;
            ...
            if (!submit) {
                alert("<?php echo JText::_('CP.HOTEL_ERROR_EMPTY_PARAM3', true); ?>");
                return;
            }
    ...
    <table border="0">
        <tr>
            <td valign="top">
                <table border="0">
                    <tr>
                        <td align="left" class="key2">
                            <label for="product_name">
                                <?php echo JText::_('CP.HOTEL_NAME'); ?>*
                            </label>
                        </td>
                        <td colspan="7">
                    ...
                    <tr>
                        ...
                        <td align="right" class="key2">
                            <label for="details_zone_id">
                                <?php echo JText::_('CP.ZONE'); ?>
                            </label>
                        </td>
                        <td>
                            <?php echo $data->zones; ?>
            ...
            <td valign="top">
                ...
                $content .= '<tr><td class="paramlist_key"><span class="editlinktip">' . JText::_('CP.HOTELPARAM1') . '*</span></td>';
                ...
                $content .= '</div></td></tr>';
                $content .= '<tr><td class="paramlist_key"><span class="editlinktip">' . JText::_('CP.HOTELPARAM2') . '*</span></td>';
                $content .= '<td class="paramlist_value"><div>';
                ...
                $content .= '</div></td></tr>';
                $content .= '<tr><td class="paramlist_key"><span class="editlinktip">' . JText::_('CP.HOTELPARAM3') . '*</span></td>';
                $content .= '<td class="paramlist_value"><div>';
                ...
                // Se inicia el panel de amenities
                $content .= $pane->startPanel(JText::_('CP.HOTELAMENITY'), "amenities-page");
                $content .= '<table width="100%" class="paramlist admintable" cellspacing="1">';
                $content .= '<tr><td width="40%" class="paramlist_key"><span class="editlinktip">' . JText::_('CP.HOTELAMENITY') . '</span></td>';
                $content .= '<td class="paramlist_value">';
                $content .= '<div class="paramsdiv">';
		        if (is_array($this->amenities) && count($this->amenities) > 0) {
		            foreach ($this->amenities as $key => $row) {
		            	// Si el amenity/facility está inactivo, mostrar sólo en los productos que ya lo tienen asignado.
		                if (in_array($row->amenity_id, $data->amenities)) {
		                ...
		                $content .= '<br /><input type="checkbox" name="amenities[]" id="amenities" value="' . $row->amenity_id . '"' . $checked . ' />&nbsp;' . $row->amenity_name;
		            }
		        }
                $content .= '</div>';
                $content .= '</td></tr>';
                ...
                <td rowspan="6" width="50%" valign="top">
                    ...
                    <div class="panel">
                        ...
                        <div style="padding: 10px;">
                            <a id="cargar_imagen" rel="{handler: 'iframe', size: {x: 570, y: 450}}" 
                               href="<?php echo JURI::base(); ?>index.php?option=com_media&view=images&tmpl=component&asset=com_catalogo_planes&author=&e_name=aqui&parent=" class="modal-button"><img src="<?php echo JURI::base(); ?>components/com_media/images/folderup_32.png" /><br /><?php echo JText::_('CP.PRODUCT_IMAGES_DESC'); ?></a>
		...
		$content .= '<div class="supplement_info"><hr />
		  <table border="0" cellpadding="5">
		      <tr>
	              ...
	              $addTaxImg . '</td></tr>' . $supplement_taxes . '</table></div></fieldset></td>
              </tr>
              <tr>
	              <td colspan="2"><input type="checkbox" name="supplements[' . $id . '][apply_once]" id="supplements[' . $id
		           . (($row->apply_once == 0)? '][apply_once]" checked="checked': '][apply_once]') . '" value="0" />
	              <label for="supplements[' . $id . '][apply_once]">' . $applyOnceLabel . '</label></td>
	              <td align="right">' . $deleteSupplementLabel . '</td>
		      </tr>
		  </table>
		</div>';
	}
...
?>
        <fieldset>
            <legend><?php echo JText::_('CP.HOTEL_SUPPLEMENT_SEARCH_LABEL'); ?></legend>
