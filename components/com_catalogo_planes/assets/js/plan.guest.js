jQuery(document).ready(function(){
	...
	//Inician funciones para autocomplete de nacionalidad
    var rowEmpty = new Array("",nationalityMinchars);

	jQuery('.nationality_autocomplete').each(function() {
	    jQuery(this).data("minLet", {
	        data: rowEmpty,
	        value: rowEmpty[0],
	        result: rowEmpty[1]
	    });		
	    var elId = jQuery(this).attr("id");
		 jQuery(this).autocomplete(urlBase+'index.php', {
			 	minChars: 3,
		        width: 255,
		        delay: 0,
		        cacheLength: 100,
				...
				click:function(){
					if(jQuery(".ui-dialog").find("#to_mail").val()!=""){
						if(isUserEmailMPA(jQuery(".ui-dialog").find("#to_mail").val())){
							jQuery.ajax({
								type: "POST",
							  	url: "index.php?option=com_catalogo_planes&view=plan&layout=maildetail",
							  	data: { 
	...
	jQuery("#frm_guest").validate({
		...
		submitHandler: function(form){
			...
			if(noRegister && requireRegister=='1'){
				//Si no se muestra el formulario de logueo
