 ...
 *
 * @version $Id: hotels.php 2012-09-11 8:29:50 svn $
 ...
 * @subpackage catalogo_planes
 */
class CatalogoPlanesControllerHotels extends JController {
	...
	function display() {
		...
		$tourismtype_id = $mainframe->getUserStateFromRequest($filter_prefix . 'tourismtype_id', 'tourismtype_id', '', 'int');
		...
		$countries = $countryModel->getCountriesList($productType);//$countryModel->getList(true, array('published' => 1));
		...
		if ($country_id > 0) {
			...
			$regions=$countryModel->getRegionsList($regionConditions,$productType);//$regions = $regionModel->getList(true, $regionConditions);
		...
		if ($country_id > 0) {
			...
			$cities = $countryModel->getCitiesList($cityConditions,$productType);//$cityModel->getList(true, $cityConditions);
		...
		// Generar lista de tipos de turismo
		$opt = JHTML::_('select.option', '', ' - ' . JText::_('CP.TOURISMTYPE') . ' - ', 'tourismtype_id', 'tourismtype_name');
		// Se obtiene el modelo de la región
		$tourismtypeModel =& $this->getModel('tourismtype');
		$tourismtypes = $tourismtypeModel->getList(false);
		array_unshift($tourismtypes, $opt);
		$lists['tourismtypes'] = JHTML::_('select.genericlist', $tourismtypes, 'tourismtype_id', 'class="inputbox" onchange="submitform();" size="1"', 'tourismtype_id', 'tourismtype_name', $tourismtype_id);

		// Generar lista de tipos de alojamientos
		$opt = JHTML::_('select.option', '', ' - ' . JText::_('CP.HOTELCATEGORY') . ' - ', 'category_id', 'category_name');
		// Se obtiene el modelo de la región
		$categoryModel =& $this->getModel('hotelcategory');
	...
	function edit() {
		...
		if ($id && empty($data->product_name)) {
			$this->setRedirect('index.php?option=' . $option . '&view=hotels', JText::_('CP.ROW_UNKNOWN'), 'error');
		}
		...
		if ($params->cfg_use_regions_hotel == '1') { 
		...
		$param1Model =& $this->getModel('hotelparam1');
		$rateparams1 = $param1Model->getList(false);
		$view->assignRef('rateparams1', $rateparams1);

		$param2Model =& $this->getModel('hotelparam2');
		$rateparams2 = $param2Model->getList(false);
		$view->assignRef('rateparams2', $rateparams2);

		$param3Model =& $this->getModel('hotelparam3');
		$rateparams3 = $param3Model->getList(false);
		$view->assignRef('rateparams3', $rateparams3);

		// Generar lista de amenities
		// Si es un nuevo producto, traer sólo las activas.
		// De lo contrario, tráigalas todas
		$amenitiesModel =& $this->getModel('hotelamenity');
		$amenities = $amenitiesModel->getList(false);
		$view->assignRef('amenities', $amenities);
		...
		$taxes = $taxesModel->getList(true, array('published' => 1, 'product_type_code' => 'hotels'));
		$view->assignRef('taxes', $taxes);

		// Generar lista de zonas
		$opt = JHTML::_('select.option', '', $emptySelectOption, 'zone_id', 'zone_name');
		$zonesModel =& $this->getModel('zone');
		$zones = $zonesModel->getList(false);
		if (!empty($zones)) {
			foreach ($zones as $key => $zone) {
				if ($zone->published != 1) {
					$zones[$key]->zone_name = $zone->zone_name . '*';
				}
			}
			// Se agrega opción vacía al inicio
			array_unshift($zones, $opt);
		} else {
			$zones = array($opt);
		}
		$data->zones = JHTML::_('select.genericlist', $zones, 'detailszone_id', 'class="inputbox" size="1"', 'zone_id', 'zone_name', $data->zone_id);
	...
	function save() {
		...
		// Redireccionar
		$link = 'index.php?option=' . $option . '&view=hotels';
		if ($this->task == 'apply') {
	...
	function remove() {
		...
		$this->setRedirect('index.php?option=' . $option . '&view=hotels', $msg, $type);
	}
	...
	function orderdown() {
		...
		$model =& $this->getModel('hotels');
		$model->move($cid, 1);
		$this->setRedirect('index.php?option=' . $option . '&view=hotels');
	}
	...
	function access() {
		...
		$row =& JTable::getInstance('hotels', 'Table');
	...
	function listrates() {
		...
		// Redireccionar
		$this->setRedirect('index.php?option=' . $option . '&view=hotelrate&product_id=' . $cid . '&season_year=' . date('Y'));
	}
	...
	function autoloadstock() {
		...
		if (count($params) < 1) {
			JError::raiseError(500, JText::_('CP.HOTEL_ERROR_EMPTY_PARAM1'));
		}
