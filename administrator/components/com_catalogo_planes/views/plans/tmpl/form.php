...
$deleteHotelImg = '<img border="0" class="deletehotel" alt="' . JText::_('CP.DELETE') . '" src="' . COM_CATALOGO_PLANES_IMAGESDIR . '/cancel.png" />';
...
<script type="text/javascript">
                ...
                var newContent = '<div class="supplement_info"><hr />' +
	                '<table border="0" cellpadding="5">' +
	                    '<tr>' +
	                        ...
	                        '<td><input type="hidden" name="supplements[' + id + '][supplement_id]" value="' + id + '" /><label for="supplement_name' + id + '"><?php echo $nameLabel; ?></label></td>' +
	...
	jQuery.noConflict()(document).ready(function ($) {
        ...
        if ($data->product_id < 1) {
            echo '$(\'#details_disclaimer\').val("' . JText::_('CP.PLAN_DISCLAIMER_DEFAULT_VALUE') . '");';
        }
        ?>
        ...
        var elementsOrder = ["product_name", "detailscountry_id", "detailsregion_id", "detailscity_id",
                             "details_publish_up", "details_publish_down",
                             "details_featured", "details_product_desc", "details_product_code",
                             "details_disclaimer", "detailstourismtypes", "details_access",
                             "detailscategory_id", "detailssupplier_id", "details_published1",
                             "details_published0", "details_duration", "details_days_total",
                             "details_product_url", "rateparam2", "selecthotels", "rateparam3", "selecttaxes"];
        ...
        $("#supplement_product_types,#supplement_tourism_types").change(function() {
            ...
            generateSelectByAjax(url, 'supplement_list', 'supplement_id', 'supplement_name', '0', emptySelectOption, assignedSupplementList, "supplement_search_message", "<?php echo JText::_('CP.PLAN_SUPPLEMENT_SEARCH_NO_RESULTS_MESSAGE', true); ?>");
        });
        ...
        var noResultsHotelMessage = new Array();
        noResultsHotelMessage.push({
                label: "<?php echo JText::_('CP.AUTOCOMPLETE_NO_MATCHES_DESCRIPTION'); ?>",
                value: 0
            });
        
        // Autocomplete de ciudades de entrega
        $("#selecthotels").autocomplete({ 
            source: function(request, response) {
                $.ajax({
                    ...
                    data: {
                        ...
                        view: "planparam1",
                        task: "getrawlistbyname",
                        format: "raw",
                        param1_name: request.term
                    },
                    success: function(data) { 
                        // Si no se obtienen resultados o todas ya están incluídos, se muestra el mensaje de resultado vacío.
                        if (data.length > 0) {
                            // Agregar SÓLO si el hotel no se haya relacionado al producto.
                            arr = $.grep(data, function(item, index) {
                                return ($('div.hotelscontent input:hidden[value="' + item.param1_id + '"]').length < 1);
                            });
                            if (arr.length > 0) {
                                response($.map(arr, function(item) {
                                    if (item.published < 1) {
                                        item.param1_name = item.param1_name + '*';
                                    }
                                    return {
                                        label: item.param1_name,
                                        value: item.param1_id
                                    };
                                }));
                            } else {
                                response(noResultsHotelMessage);
                            }
                        } else {
                            response(noResultsHotelMessage);
                        }
            ...
            select: function(event, ui) {
                // Evitar que lance el evento por defecto que reemplaza el texto en el campo de búsqueda por el valor seleccionado del listado.
                event.preventDefault();

                if (ui.item.value != 0) {
                    $('div.hotelscontent table').append('<tr><td><input type="hidden" name="rateparam1[]" id="rateparam1[]" value="' + ui.item.value + '" /><span>' + ui.item.label + '</span></td><td align="right"><?php echo $deleteHotelImg; ?></td></tr>');

                    // Hacer que el panel de ciudades se pueda extender o encoger a medida que se agregar/eliminan
                    $('#rate-page ~ div.jpane-slider').css('height', 'auto');
        ...
        // Eliminar ciudad de entrega del producto
        $("body").delegate(".deletehotel", "click", function() {
            var row = $(this).closest("tr");

            // Eliminar la fila de ciudad.
            row.remove();
...
Joomla.submitbutton = function(pressbutton) {
        ...
        if (jQuery.trim(form.product_name.value) == "") {
        ...
        } else if (form.detailscategory_id.selectedIndex == 0) {
            alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_PLANCATEGORY', true); ?>");
            form.detailscategory_id.focus();
        ...
        } else if (jQuery.trim(form.details_duration.value) == "") {
            alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_DURATION', true); ?>");
            form.details_duration.focus();
        } else if (jQuery.trim(form.details_days_total.value) == "") {
            alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_DAYS_TOTAL', true); ?>");
            form.details_days_total.focus();
        } else {
            var reg = /[^0123456789]/g;
            if (reg.test(jQuery.trim(form.details_days_total.value))) {
                alert("<?php echo JText::_('CP.PLAN_ERROR_INVALID_DAYS_TOTAL', true); ?>");
                form.details_days_total.focus();
                return;
            }
            ...
            var submit = false;
            for (var i = 0; i < form['rateparam2[]'].length; i++) {
                if (form['rateparam2[]'][i].selected) {
                    submit = true;
                    break;
                }
            }
            if (!submit) {
                alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_PARAM2', true); ?>");
                return;
            }
            if (!form['rateparam1[]']) {
                alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_PARAM1', true); ?>");
                return;
            }
            submit = false;
            ...
            if (!submit) {
                alert("<?php echo JText::_('CP.PLAN_ERROR_EMPTY_PARAM3', true); ?>");
                return;
            }
    ...
    <table border="0">
        <tr>
            ...
            <td valign="top">
                ...
                $content .= '<table id="rate-table" class="paramlist admintable" cellspacing="1">';
                ...
                $content .= '<tr><td class="paramlist_key"><span class="editlinktip">' . JText::_('CP.PLANPARAM2') . '*</span></td>';
                $content .= '<td class="paramlist_value"><div>';
                $content .= JHTML::_('select.genericlist', $this->rateparams2, 'rateparam2[]', 'class="multiselect" size="8" multiple="multiple"', 'param2_id', 'param2_name', $data->rateparam2);
                $content .= '</div></td></tr>';
                $content .= '<tr><td class="paramlist_key"><span class="editlinktip">' . JText::_('CP.PLANPARAM1_LABEL') . '*</span></td>';
                $content .= '<td class="paramlist_value">';
                $selectedHotels = '';
                // Generar listado de hoteles ya asignados
                foreach ($data->rateparam1 as $key => $row) {
                    // El nombrel hotel debe contener asterisco si está inactivo
                    $row->param1_name .= ($row->published != 1)? '*&nbsp;': '&nbsp;';
                    $selectedHotels .= '<tr><td><input type="hidden" name="rateparam1[]" id="rateparam1[]" value="' . $row->param1_id . '" /><span>' . $row->param1_name . '</span></td><td align="right">' . $deleteHotelImg . '</td></tr>';
                }
                $content .= JText::_('CP.PLAN_PARAM1_DESCRIPTION') . '<br />';
                $content .= '<div class="hotelscontent"><table border="0" class="hotelautocomplete"><tr><td colspan="2">';
                $content .= '<input class="text_area" type="text" name="selecthotels" id="selecthotels" size="40" value="" />';
                $content .= '</td></tr>' . $selectedHotels . '</table></td></tr>';
                $content .= '<tr><td class="paramlist_key"><span class="editlinktip">' . JText::_('CP.PLANPARAM3') . '*</span></td>';
                $content .= '<td class="paramlist_value"><div>';
                $content .= JHTML::_('select.genericlist', $this->rateparams3, 'rateparam3[]', 'class="multiselect" size="8" multiple="multiple"', 'param3_id', 'param3_name', $data->rateparam3);
                $content .= '</div></td></tr>';
                $content .= '</table>';
                $content .= $pane->endPanel();
...
if (count($data->supplements)) {
	foreach ($data->supplements as $key => $row) {
        ...
        $content .= '<div class="supplement_info"><hr />
          <table border="0" cellpadding="5">
              <tr>
                  <td><table border="0" cellpadding="5"><tr><td><input type="hidden" name="supplements[' . $id . '][supplement_id]" value="' . $id . '" /><label for="supplement_name' . $id . '">' . $nameLabel . '</label></td>
...
<hr>
        <fieldset>
            <legend><?php echo JText::_('CP.PLAN_SUPPLEMENT_SEARCH_LABEL'); ?></legend>
